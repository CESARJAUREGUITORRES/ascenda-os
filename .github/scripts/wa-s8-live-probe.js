...
(async()=>{
  let x=await text('/health?probe=wa-s8-node-20260817');
  ok(x.r.ok,'health '+x.r.status);
  ok(x.body.includes('ascenda-phase-s'),'Phase S marker missing');
  x=await text('/app?probe=wa-s8-node-20260817');
  ...
  x=await text('/wa-native-panel.js?probe=wa-s8-node-20260817');
  ...
  ok(x.body.includes('wa8-right-body')&&x.body.includes('wa8-route'),'Lead/Routing structure missing');
  ...
  x=await text('/wa-shell-integration.js?probe=wa-s8-node-20260817');
  ...
  x=await text('/api/wa3/bootstrap?probe=wa-s8-node-20260817');
