...
select throws_ok($$insert into public.aos_f5_source_batches_v1(source_sha256,source_filename,source_sede,source_year,source_rows,source_columns,schema_hash) values('sha-test-1','dup.xlsx','SAN ISIDRO',2024,1,27,'schema-1')$$,'23505',null,'16 source SHA is idempotent');
...
select throws_ok($$insert into public.aos_f5_patient_source_rows_v1(batch_id,source_row_num,source_patient_id,row_content_hash,raw_payload) select id,2,'456','rowhash-2','{}'::jsonb from public.aos_f5_source_batches_v1 where source_sha256='sha-test-1'$$,'23505',null,'18 duplicate source row blocked');
