...
const e=fs.readFileSync('supabase/migrations/20260916055000_booking_v31_admin_notification_relabel.sql','utf8');
...
ok(d.includes("'source_label',src_label")&&d.includes("'ADMIN_APPOINTMENT_DIGEST'"),'admin notification source metadata missing');
ok(d.includes("'APPOINTMENT_CREATED'")&&d.includes("'TEAM_APPOINTMENT_SCORE'"),'existing notification paths missing');
ok(e.includes('deferrable initially deferred')&&e.includes('aos_booking_notification_relabel_v1'),'source-aware notification relabel missing');
