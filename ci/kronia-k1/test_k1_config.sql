...
begin
  ...
  -- Owner and level-2 sessions use real synthetic 2FA proofs.
  select id into owner_id from public.aos_usuarios where codigo_asesor='A001';
  update public.aos_usuarios set two_factor=true,activo=true,nivel_jerarquia=1,rol='admin' where id=owner_id;
  owner_token:=public.k1_ci_claim_token('alice','alice-pass');
  ...
  if level2_id is null then
    ...
    update public.aos_usuarios set activo=true,two_factor=true,nivel_jerarquia=2,rol='admin' where id=level2_id;
  end if;
  ...
  -- 2FA cannot be disabled even by owner through the gateway.
  ...
  -- K1-103..105: database invariant itself blocks disabling/deleting global 2FA.
  blocked:=false;
  ...
  if not blocked then raise exception 'K1-103 DB invariant allowed 2FA=false'; end if;
  blocked:=false;
  begin delete from public.aos_configuracion where clave='seg_2fa_habilitado'; exception when others then blocked:=true; end;
  if not blocked then raise exception 'K1-104 DB invariant allowed 2FA row deletion'; end if;
  select valor into v from public.aos_configuracion where clave='seg_2fa_habilitado';
  if lower(coalesce(v,''))<>'true' then raise exception 'K1-105 2FA invariant did not preserve true'; end if;
end $$;
