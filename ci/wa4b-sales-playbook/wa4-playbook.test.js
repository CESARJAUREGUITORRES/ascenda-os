...
function ctx(id,type,name,price,ready=true){return {entity_id:id,entity_type:type,entity_name:name,commercial_phase_codes:type==='PRODUCTO'?['COMMERCIAL_F3_CONTINUITY']:['COMMERCIAL_F2_INTERVENTION'],quote_price:price,price_state:ready?'READY':'REVIEW_REQUIRED_OFFER_ABOVE_BASE',freshness_state:'FRESH',ready_for_quote:ready,price_evidence_ref:'aos_catalogo_servicios:'+id+':v1'};}
...
test('price playbook is ready only with governed rules plus 1C ready price context',()=>{
  ...
  const out=p.buildPlaybook({inbound:'¿Cuánto cuesta SERVICIO TEST?',publicBundle,advisorBundle,processContexts:[ctx(SERVICE_ID,'SERVICIO','SERVICIO TEST',450)]});
  assert.equal(out.status,'READY');
