...
values
 ...
 ('P9','Fused','Historical','999900009','p9@example.test','90000009','SAN ISIDRO','TEST','2025-01-01','FUSIONADO','999900009')
...
select * from (
  ...
  union all select 104,b.bid,104,'S-P9-H','999900009','90000009','p9@example.test','2025-12-01'::date from b
) x
...
insert into public.aos_f5_identity_clusters_v1(id,status,confidence,source_row_count) values
 ...
 ('99999999-9999-9999-9999-999999999999','AUTO_CANDIDATE','HIGH',1)
on conflict (id) do nothing;
...
insert into public.aos_f5_identity_cluster_members_v1(cluster_id,source_row_id,match_rule,match_score) values
 ...
 ('99999999-9999-9999-9999-999999999999',104,'TEST',100);
...
insert into public.aos_f5_canonical_classification_v1(cluster_id,target_patient_id,source_match_status,classification,reason,match_method,match_score) values
 ...
 ('99999999-9999-9999-9999-999999999999','P9','AUTO_CANDIDATE','MATCH','TEST','DNI_NAME+PHONE_NAME',100)
...
insert into public.aos_agenda_citas(id,fecha_cita,hora_cita,tratamiento,tipo_cita,sede,numero,numero_limpio,estado_cita,asesor) values
 ...
 ('F62-P9-1','2026-08-10','09:00','TEST','CONTROL','SAN ISIDRO','999900009','999900009','ASISTIO','ASESOR')
on conflict (id) do nothing;
