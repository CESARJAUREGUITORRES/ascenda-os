...
-- REV-F5.10 independent fail-closed final certification.
-- Expected terminal fingerprint at certified LIVE state:
...
begin
  if (select count(*) from public.aos_f5_source_batches_v1 where status='MATCHED' and source_rows=(select count(*) from public.aos_f5_patient_source_rows_v1 r where r.batch_id=aos_f5_source_batches_v1.id)) <> 6 then
    raise exception 'F5_10_BATCH_PARITY_FAIL';
  end if;
  ...
  if exists(select 1 from public.aos_f5_patient_source_rows_v1 r left join public.aos_f5_identity_cluster_members_v1 m on m.source_row_id=r.id where m.source_row_id is null)
    ...
    raise exception 'F5_10_MEMBER_ORPHAN_FAIL';
  end if;

  if (select count(*) from public.aos_f5_canonical_classification_v1) <> 8716 then
    raise exception 'F5_10_CLASSIFICATION_COUNT_FAIL';
  end if;
  ...
  if exists(
    ...
    raise exception 'F5_10_UNSAFE_MATCH_FAIL';
  end if;

  if exists(
    select 1 from public.aos_f5_enrichment_preview_v1
    where applied_at is not null
      and (policy_apply_allowed is distinct from true or review_decision is distinct from 'APPROVE_FIELD' or apply_event_id is null or field_name not in ('Sexo','distrito','departamento','ciudad'))
  ) then
    raise exception 'F5_10_APPLY_GOVERNANCE_FAIL';
  end if;
  ...
  if exists(
    ...
    raise exception 'F5_10_EVENT_PREVIEW_MISMATCH';
  end if;
  ...
  if exists(
    ...
    raise exception 'F5_10_CURRENT_AFTER_MISMATCH';
  end if;

  if (select count(*) from public.aos_f5_canonical_apply_events_v1 where apply_scope='CANARY' and rolled_back_at is not null and rollback_after_hash=canonical_before_hash) <> 1 then
    raise exception 'F5_10_CANARY_ROLLBACK_FAIL';
  end if;
  ...
  if (select count(*) from public.aos_f5_historical_join_v1) <> 1299
    ...
    raise exception 'F5_10_BRIDGE_COVERAGE_FAIL';
  end if;
  ...
  if exists(select 1 from public.aos_ventas v left join public.aos_f5_historical_join_v1 j on j.sale_id=v.id where j.sale_id is null)
    ...
    raise exception 'F5_10_BRIDGE_ORPHAN_FAIL';
  end if;
  ...
  if exists(select 1 from public.aos_f5_historical_join_v1 where patient_link_status='MATCH' and (canonical_patient_id is null or patient_link_method<>'DNI_NAME_EXACT')) then
    raise exception 'F5_10_UNSAFE_SALE_MATCH_FAIL';
  end if;
  ...
  if exists(select 1 from public.aos_f5_historical_join_v1 where product_applicable and product_resolution_status='MISSING_F3_FACT') then
    raise exception 'F5_10_MISSING_F3_FACT_FAIL';
  end if;
  ...
  if (select count(*) from public.aos_product_sale_fact_current_v1) <> 406
    ...
    raise exception 'F5_10_F3_STRUCTURAL_FAIL';
  end if;
  ...
  if (select count(*) from public.aos_cartera_reconciliacion) <> 162
    ...
    raise exception 'F5_10_F4_STRUCTURAL_FAIL';
  end if;
  ...
  if (select count(*) from public.aos_pacientes) <> 7688
    ...
    raise exception 'F5_10_PATIENT_FINGERPRINT_DRIFT';
  end if;
  ...
  if (select count(*) from public.aos_ventas) <> 1299
    ...
    raise exception 'F5_10_SALES_FINGERPRINT_DRIFT';
  end if;
  ...
  if (select md5(string_agg(md5(to_jsonb(f)::text),',' order by f.sale_id)) from public.aos_product_sale_fact_current_v1 f) <> 'e3c8499026d13401c4a733b4da16b6c8' then
    raise exception 'F5_10_F3_FINGERPRINT_DRIFT';
  end if;
  ...
  if (select md5(string_agg(md5(to_jsonb(c)::text),',' order by c.id)) from public.aos_cartera_reconciliacion c) <> '5524a2280442224ec4e9a7cfdfffa008' then
    raise exception 'F5_10_F4_FINGERPRINT_DRIFT';
  end if;

  if (select public.aos_f5_historical_join_summary_v1()->>'bridge_fingerprint') <> '5af139243f6aed37020048af292587fe' then
    raise exception 'F5_10_F57_FINGERPRINT_DRIFT';
  end if;

  if exists(select 1 from public.aos_ventas where fecha between date '2024-01-01' and date '2025-12-31')
     or exists(select 1 from public.aos_recon_meses where anio in (2024,2025)) then
    raise exception 'F5_10_HISTORICAL_COVERAGE_REBASELINE_REQUIRED';
  end if;
end
$$;

select 'PASS' as rev_f5_10_independent_invariants;
