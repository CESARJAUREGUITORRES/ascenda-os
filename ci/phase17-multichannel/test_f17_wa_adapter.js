...
(async()=>{
  ...
  const prepared=await adapter.prepareOutbound({actor:'33333333-3333-4333-8333-333333333333',idempotencyKey:'runtime-canary-0001',payload:{to:'51999111222',type:'text',text:{body:'secret body must not enter F17 facts'}}});
  ...
  await adapter.ingestEnvelope({messages:[{provider_message_id:'wamid.in',from_number:'51999888777',message_type:'text',message_body:'DO NOT COPY',raw_referral:{body:'DO NOT COPY REF'},campaign_source:'campaign',ad_id:'ad1',lead_id:'lead1',provider_timestamp:'2026-08-16T00:00:00Z'}],events:[{event_key:'message:wamid.in',event_type:'message.received',provider_message_id:'wamid.in',status:'received',payload:{message_type:'text'}}]});
  ...
  const nonCanary=createF17WaAdapter({canaryMode:'false',serviceRpc:async(name,args)=>{nonCanaryCalls.push({name,args});if(name==='aos_cia_channel_prepare_send_v1')return {ok:true,state:'BLOCKED',dispatch_allowed:false};throw new Error('unexpected noncanary rpc '+name);}});
