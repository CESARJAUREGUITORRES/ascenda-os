...
do $$ declare r jsonb; tid uuid; d date; begin
 ...
 r:=public.aos_wa_l5_availability_v1('55555555-5555-4555-8555-555555555551'::uuid,tid,'SAN ISIDRO',d,'wa4c-doctor-1','DOCTORA',3);
...
do $$ declare r jsonb; tid uuid; d date; begin
 select treatment_id,target_date into tid,d from public.aos_wa4c_booking_test_fixture where id=1;
 r:=public.aos_wa_l5_prepare_confirmation_v1('55555555-5555-4555-8555-555555555551'::uuid,'BOOK',tid,'SAN ISIDRO',d,'10:00'::time,'wa4c-doctor-1','DOCTORA',null,'CLIENTE','BOOK',600);
...
values('wamid.l5.book.no','55555555-5555-4555-8555-555555555551'::uuid,'INBOUND','51911111111','51999999999','local-phone-id','text','Todavía no','received',now(),now())
on conflict(provider_message_id) do nothing;

do $$ declare r jsonb; n uuid; begin
 select confirmation_nonce into n from public.aos_wa_l5_booking_memory_v1 where conversation_id='55555555-5555-4555-8555-555555555551'::uuid;
 r:=public.aos_wa_l5_mark_explicit_confirmation_v1('55555555-5555-4555-8555-555555555551'::uuid,n,'wamid.l5.book.no');
...
do $$ declare r jsonb; n uuid; begin
 select confirmation_nonce into n from public.aos_wa_l5_booking_memory_v1 where conversation_id='55555555-5555-4555-8555-555555555551'::uuid;
 r:=public.aos_wa_l5_mark_explicit_confirmation_v1('55555555-5555-4555-8555-555555555551'::uuid,n,'wamid.l5.book.yes');
...
do $$ declare r jsonb; n uuid; before_n bigint; after_n bigint; begin
 ...
 select confirmation_nonce into n from public.aos_wa_l5_booking_memory_v1 where conversation_id='55555555-5555-4555-8555-555555555551'::uuid;
 r:=public.aos_wa_l5_commit_confirmed_v1('55555555-5555-4555-8555-555555555551'::uuid,n,'wa-l5-book-idem-000001');
...
do $$ declare r jsonb; tid uuid; d date; begin
 select treatment_id,target_date into tid,d from public.aos_wa4c_booking_test_fixture where id=1;
 r:=public.aos_wa_l5_prepare_confirmation_v1('55555555-5555-4555-8555-555555555553'::uuid,'BOOK',tid,'SAN ISIDRO',d,'11:00'::time,'wa4c-doctor-1','DOCTORA',null,'CONFLICT','TEST',600);
 ...
 if (select state from public.aos_wa_l5_booking_memory_v1 where conversation_id='55555555-5555-4555-8555-555555555553'::uuid)<>'HANDOFF' then raise exception 'L5_IDENTITY_CONFLICT_STATE'; end if;
end $$;

-- REBOOK requires exact second-factor document verification and exposes only governed appointments.
do $$ declare r jsonb; begin
 r:=public.aos_wa_l5_verify_patient_v1('55555555-5555-4555-8555-555555555552'::uuid,'00000000');
 if r->>'error'<>'WA_L5_VERIFICATION_FAILED' then raise exception 'L5_BAD_DOCUMENT_ACCEPTED %',r; end if;
 r:=public.aos_wa_l5_verify_patient_v1('55555555-5555-4555-8555-555555555552'::uuid,'12345678');
 if coalesce((r->>'ok')::boolean,false) is not true or r->>'status'<>'VERIFIED' then raise exception 'L5_VERIFY %',r; end if;
 r:=public.aos_wa_l5_active_appointments_v1('55555555-5555-4555-8555-555555555552'::uuid);
...
do $$ declare r jsonb; begin
 r:=public.aos_wa_l4_allowlist_set_v1('11111111-1111-4111-8111-111111111111'::uuid,'PHONE','51911111111',true,null,'WA-L5 LOCAL BOOK');
 if coalesce((r->>'ok')::boolean,false) is not true then raise exception 'L5_ALLOWLIST_BOOK %',r; end if;
 r:=public.aos_wa_l4_allowlist_set_v1('11111111-1111-4111-8111-111111111111'::uuid,'PHONE','51933333333',true,null,'WA-L5 LOCAL REBOOK');
 if coalesce((r->>'ok')::boolean,false) is not true then raise exception 'L5_ALLOWLIST_REBOOK %',r; end if;
 r:=public.aos_wa_l4_set_control_v1('11111111-1111-4111-8111-111111111111'::uuid,'CANARY',false,50,12,60,10,0,0,'WA-L5-LOCAL-CANARY-ONLY');
...
do $$ declare r jsonb; n uuid; begin
 ...
 if (select state from public.aos_wa_l5_booking_memory_v1 where conversation_id='55555555-5555-4555-8555-555555555551'::uuid)<>'COMMITTED' then raise exception 'L5_BOOK_MEMORY_NOT_COMMITTED'; end if;
...
do $$ declare r jsonb; tid uuid; d date; n uuid; before_id text:='L5-REBOOK-APPT-1'; begin
 select treatment_id,target_date into tid,d from public.aos_wa4c_booking_test_fixture where id=1;
 r:=public.aos_wa_l5_prepare_confirmation_v1('55555555-5555-4555-8555-555555555552'::uuid,'REBOOK',tid,'SAN ISIDRO',d,'11:00'::time,'wa4c-doctor-1','DOCTORA',before_id,null,null,600);
 ...
 select confirmation_nonce into n from public.aos_wa_l5_booking_memory_v1 where conversation_id='55555555-5555-4555-8555-555555555552'::uuid;
 insert into public.aos_wa_messages_v1(provider_message_id,conversation_id,direction,from_number,to_number,phone_number_id,message_type,message_body,status,provider_timestamp,received_at)
 values('wamid.l5.rebook.yes','55555555-5555-4555-8555-555555555552'::uuid,'INBOUND','51933333333','51999999999','local-phone-id','text','Confirmo','received',now(),now())
 on conflict(provider_message_id) do nothing;
 r:=public.aos_wa_l5_mark_explicit_confirmation_v1('55555555-5555-4555-8555-555555555552'::uuid,n,'wamid.l5.rebook.yes');
 if coalesce((r->>'ok')::boolean,false) is not true then raise exception 'L5_REBOOK_CONFIRM %',r; end if;
 r:=public.aos_wa_l5_commit_confirmed_v1('55555555-5555-4555-8555-555555555552'::uuid,n,'wa-l5-rebook-idem-0001');
...
do $$ declare r jsonb; begin
 r:=public.aos_wa_l4_set_control_v1('11111111-1111-4111-8111-111111111111'::uuid,'AUTO_OFF',true,null,null,null,null,null,null,null);
