...
f4=(app/'server-f4.js').read_text(encoding='utf-8')
phase2=(app/'server-phase2.js').read_text(encoding='utf-8')
...
assert "spawn(process.execPath,['server-f4.js']" in proxy
assert 'PHASE2_INTERNAL_PORT' in proxy and "PORT:String(F4_PORT)" in proxy
assert "spawn(process.execPath,['server-phase2.js']" in f4
assert "spawn(process.execPath, ['server.js']" in phase2
...
assert 'window._SK' in browser and 'aos_app_token' in browser
...
assert 'scrubCredentialEmail' in browser
...
assert 'Mínimo 6 caracteres' not in team and 'mínimo 6 caracteres' not in team
...
assert manifest['contract']=='kronia-k1-current-runtime-v2'
assert 'server-f4.js' in manifest['files'] and 'public/admin-team.html' in manifest['files']
assert all(len(v)==64 for v in manifest['files'].values())
print('KRONIA_K1_SOURCE_SECRET_CONTRACT=PASS')
print('KRONIA_K1_F4_TOPOLOGY=PASS')
