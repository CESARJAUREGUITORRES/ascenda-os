...
const convL2OwnsLock=lock.includes('**ACTIVE HIGH/CRITICAL LOCK:** `CONV-L2 #506 — CONVERSATION CORE + EVENT-DRIVEN PANEL TRANSPORT`')
const googleOwnsLock=lock.includes('**ACTIVE HIGH/CRITICAL LOCK:** `INT-GOOGLE-001 / GC-0/GC-1 — GOOGLE CALENDAR + CONTACTS OAUTH FOUNDATION`') && lock.includes('PAUSED')
const convL4OwnsLock=lock.includes('**ACTIVE HIGH/CRITICAL LOCK:** `CONV-L4 #508 — BUSINESS TOOLS + BOUNDED RAG`') && lock.includes('INT-GOOGLE-001 #542') && lock.includes('LOCK RELEASED')
assert(convL2OwnsLock||googleOwnsLock||convL4OwnsLock, 'CONV governance state missing: expected active CONV lock or explicit Google transfer state')
