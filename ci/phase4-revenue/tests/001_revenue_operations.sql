...
insert into public.aos_ventas(fecha,nombres,celular,numero_limpio,tratamiento,descripcion,pago,monto,estado_pago,asesor,sede,tipo)
values
...
select is(public.aos_sales_admin_gateway_v4('F4-TEST-TOKEN',8,2026,'SAN ISIDRO','WILMER','MES')->'detalle'->1->>'rawDescription','LIFTIN B','5 raw product description preserved');
...
select is(public.aos_sales_admin_sale_v4('F4-TEST-TOKEN',2)->'row'->>'canonicalProductName','LIFTING B 30GR','12 secure sale read exposes canonical identity');
...
create temporary table f4_version(v timestamptz);
insert into f4_version select updated_at from public.aos_ventas where id=2;
...
create temporary table f4_preview(j jsonb);
insert into f4_preview select public.aos_importar_ventas_preview_v4('F4-TEST-TOKEN','[
 {"fecha":"2026-08-20","sede":"SAN ISIDRO","nombres":"E","celular":"999555555","tratamiento":"COMPRA DE PRODUCTO","descripcion":"LIFTIN B","pago":"QR","monto":"150","estado_pago":"ADELANTO","asesor":"WILMER","atendio":"DOC"},
 {"fecha":"2026-08-20","sede":"SAN ISIDRO","nombres":"F","celular":"999666666","tratamiento":"COMPRA DE PRODUCTO","descripcion":"NUEVO ALIAS","pago":"QR","monto":"180","estado_pago":"PAGO COMPLETO","asesor":"WILMER","atendio":"DOC"}
]'::jsonb);
select is((select (j->>'ok')::boolean from f4_preview),true,'18 import preview valid');
select is((select (j->>'mutates')::boolean from f4_preview),false,'19 preview declares read-only');
...
create temporary table f4_import(j jsonb);
insert into f4_import select public.aos_importar_ventas_v4('F4-TEST-TOKEN','[
 {"fecha":"2026-08-20","sede":"SAN ISIDRO","nombres":"E","celular":"999555555","tratamiento":"COMPRA DE PRODUCTO","descripcion":"LIFTIN B","pago":"QR","monto":"150","estado_pago":"ADELANTO","asesor":"WILMER","atendio":"DOC"},
 {"fecha":"2026-08-20","sede":"SAN ISIDRO","nombres":"F","celular":"999666666","tratamiento":"COMPRA DE PRODUCTO","descripcion":"NUEVO ALIAS","pago":"QR","monto":"180","estado_pago":"PAGO COMPLETO","asesor":"WILMER","atendio":"DOC"}
]'::jsonb);
select is((select (j->>'ok')::boolean from f4_import),true,'24 secure import succeeds');
select is((select (j->>'insertados')::integer from f4_import),2,'25 secure import reports two inserted');
select is((select count(*) from public.aos_ventas),(select n+2 from f4_count_before),'26 secure import inserts expected rows');
select is((select count(*) from public.aos_product_sale_fact_v1 where resolution_status='REVIEW_REQUIRED'),1::bigint,'27 unknown imported product fails closed to review');
...
select is((public.aos_grabar_venta_caja_v4('bad','SAN ISIDRO','spoof','CAJA-1','999777777','G','','999777777','77777777','WILMER','DOC','[{"nombre":"LIFTING B","descripcion":"LIFTIN B"}]','QR',200,'PEN','BOLETA','B1','PAGO COMPLETO','','PRODUCTO','2026-08-21',null,200)->>'error'),'UNAUTHORIZED','28 unauthorized caja sale rejected');
...
select is((select count(*) from public.aos_ventas),(select n+1 from f4_caja_before),'31 caja creates exactly one sale');

-- Cartera candidate/reconciliation: source advance + later full payment + matching quote.
insert into public.aos_ventas(fecha,nombres,dni,celular,numero_limpio,tratamiento,descripcion,pago,monto,estado_pago,asesor,sede,tipo)
values('2026-07-01','H','88888888','999888888','999888888','HIFU','HIFU','QR',300,'ADELANTO','WILMER','SAN ISIDRO','SERVICIO');
insert into public.aos_ventas(fecha,nombres,dni,celular,numero_limpio,tratamiento,descripcion,pago,monto,estado_pago,asesor,sede,tipo)
values('2026-07-12','H','88888888','999888888','999888888','HIFU','HIFU','QR',700,'PAGO COMPLETO','WILMER','SAN ISIDRO','SERVICIO');
insert into public.aos_cotizaciones(id,numero_limpio,nombre_paciente,dni_paciente,estado,subtotal,total_pagado,saldo_pendiente,sede,asesor,fecha_creacion)
values('Q-1','999888888','H','88888888','PAGADO_PARCIAL',1000,300,700,'SAN ISIDRO','WILMER','2026-07-01');
insert into public.aos_cartera_reconciliacion(id,source_type,venta_row_id,rol_pago,estado_reconciliacion,confianza,monto_registrado,source_active,evidencia,updated_at)
values('00000000-0000-0000-0000-000000000099','VENTA',(select max(id)-1 from public.aos_ventas),'ADELANTO','PENDIENTE_RECONCILIAR','NO_EVALUADA',300,true,'{}',now());
...
select is(public.aos_cartera_reconcile_v2('F4-TEST-TOKEN','00000000-0000-0000-0000-000000000099',(select updated_at from public.aos_cartera_reconciliacion where id='00000000-0000-0000-0000-000000000099'),'PAGO_RECONCILIADO','ALTA',1000,0,null,null,'ADELANTO','test')->>'error','EVIDENCE_LINK_REQUIRED','35 reconciled state requires linked evidence');
select is((public.aos_cartera_reconcile_v2('F4-TEST-TOKEN','00000000-0000-0000-0000-000000000099',(select updated_at from public.aos_cartera_reconciliacion where id='00000000-0000-0000-0000-000000000099'),'PAGO_RECONCILIADO','ALTA',1000,0,'VENTA',(select max(id)::text from public.aos_ventas),'ADELANTO','linked existing payment evidence')->>'ok')::boolean,true,'36 sale evidence reconciliation succeeds');
select is((select estado_reconciliacion from public.aos_cartera_reconciliacion where id='00000000-0000-0000-0000-000000000099'),'PAGO_RECONCILIADO','37 reconciliation state persisted');
select is((select evidencia#>>'{f4_link,type}' from public.aos_cartera_reconciliacion where id='00000000-0000-0000-0000-000000000099'),'VENTA','38 linked evidence persisted without payment creation');
