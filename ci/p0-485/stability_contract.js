...
function sourceContracts() {
  ...
  assert(lock.includes('**ACTIVE HIGH/CRITICAL LOCK:** `CONV-L4 #508 — BUSINESS TOOLS + BOUNDED RAG`'), 'CONV-L4 active lock missing')
  assert(lock.includes('RUN UNTIL BLOCKED'), 'current owner authorization missing')
  assert(lock.includes('INT-GOOGLE-001 #542') && lock.includes('LOCK RELEASED'), 'Google closeout/lock release marker missing')
