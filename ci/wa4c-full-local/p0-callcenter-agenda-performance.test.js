...
test('Loop6 booking V3 resolves PHONE identity with predicate pushdown and preserves conflict semantics',()=>{
  ...
  assert.doesNotMatch(hotExec,/from public\.aos_rev_patient_identity_alias_v2/i);
});
...
test('Loop6 booking V3 defers analytical lifecycle but keeps realtime policy evidence',()=>{
  ...
  assert.doesNotMatch(hotExec,/aos_rev_customer_lifecycle_v1\(/);
});

test('Loop6 P0 does not hide the incident by raising statement timeout',()=>{
  assert.doesNotMatch(hotExec,/statement_timeout/i);
  assert.doesNotMatch(hotExec,/set_config\([^\n]*statement_timeout/i);
  assert.doesNotMatch(hotExec,/auto_reply_enabled[\s\S]*true/i);
  assert.doesNotMatch(hotExec,/ai_send_enabled[\s\S]*true/i);
  assert.doesNotMatch(hotExec,/auto_routing_enabled[\s\S]*true/i);
});
