...
assert.strictEqual(gw._test.shouldRetryCalendarWithoutAttendee(400,[{email:'synthetic@example.invalid'}]),true,'Calendar 400 with attendee must retry without invite')
assert.strictEqual(gw._test.shouldRetryCalendarWithoutAttendee(400,[]),false,'Calendar 400 without attendee must not hide unrelated errors')
assert.strictEqual(gw._test.shouldRetryCalendarWithoutAttendee(500,[{email:'synthetic@example.invalid'}]),false,'Only provider 400 is eligible for attendee fallback')
assert.strictEqual(gw._test.shouldSkipContactSyncError('PATIENT_NOT_RESOLVED'),true,'unresolved patient identity must fail closed without retry loop')
assert.strictEqual(gw._test.shouldSkipContactSyncError('CONTACT_IDENTITY_CONFLICT'),true,'contact identity conflict must fail closed without retry loop')
assert.strictEqual(gw._test.shouldSkipContactSyncError('GOOGLE_CONTACT_UPSERT_FAILED_500'),false,'provider failures remain retryable')
