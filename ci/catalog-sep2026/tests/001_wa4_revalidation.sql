...
-- 1C authority must follow 184 services + 50 products and 9 toppings with currency.
do $$ begin
 ...
 if (select count(*) from public.aos_wa4_price_authority_v1 where moneda='USD')<>3 then raise exception 'SEP26 1C USD rows !=3'; end if;
 if exists(select 1 from public.aos_wa4_price_authority_v1 where moneda not in ('PEN','USD') or not ready_for_quote) then raise exception 'SEP26 current price authority not quote-ready'; end if;
