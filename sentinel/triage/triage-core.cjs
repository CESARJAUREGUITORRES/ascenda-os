...
function scanPrivacyPatterns(text,code,{includeNaturalPii=true}={}){
  for(const [name,pattern] of Object.entries(CONTRACT.output_privacy_patterns)){
    if(!includeNaturalPii&&NATURAL_ONLY_PATTERNS.has(name))continue;
...
function privacyCheckString(s,code='F11_SENSITIVE_OUTPUT'){
  ...
  scanPrivacyPatterns(text,code,{includeNaturalPii:true});
  return text;
}
function privacyCheckTechnicalString(s,code='F11_SENSITIVE_OUTPUT'){
  const text=String(s??'');
  scanPrivacyPatterns(text,code,{includeNaturalPii:false});
  return text;
}
...
function assertDiagnostic(report){
  ...
  for(const e of report.evidence){
    ...
    ids.add(e.id); privacyCheckTechnicalString(JSON.stringify(e));
  }
...
function buildPacket(report){
  ...
  privacyCheckTechnicalString(JSON.stringify(packet));
...
function validateResponse(response,packet){
  ...
  privacyCheckTechnicalString(JSON.stringify(validated));
  const audit={
...
module.exports={CONTRACT,buildPacket,validateResponse,digest,privacyCheckString,privacyCheckTechnicalString};
