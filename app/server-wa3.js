...
const {CORE_VERSION,createEventHub}=require('./conversation-core');
...
const core=createEventHub({heartbeatMs:20000,maxClients:250});
...
function writeJson(res,status,obj){res.writeHead(status,{'Content-Type':'application/json; charset=utf-8','Cache-Control':'no-store','X-Content-Type-Options':'nosniff','X-Ascenda-WA3-Routing':'v1','X-Ascenda-Conversation-Core':CORE_VERSION});res.end(JSON.stringify(obj));}
...
function sbRequest(method,endpoint,body,useService,prefer){return new Promise((resolve,reject)=>{
  ...
  const transport=sb.protocol==='http:'?http:https;const q=transport.request({hostname:sb.hostname,port:sb.port||(sb.protocol==='http:'?80:443),path:endpoint,method,headers,timeout:12000},r=>{let raw='';r.on('data',c=>raw+=c);r.on('end',()=>{const out={status:r.statusCode||502,data:parseData(raw),raw};if(out.status>=200&&out.status<300)resolve(out);else reject(Object.assign(new Error('WA3_DB_UNAVAILABLE'),{status:502,upstreamStatus:out.status,data:out.data}));});});q.on('timeout',()=>q.destroy(new Error('WA3_DB_TIMEOUT')));q.on('error',reject);if(data)q.write(data);q.end();
});}
...
async function bootstrap(req,res){const a=await requireActor(req,res,false);if(!a)return;try{
  ...
  writeJson(res,200,{ok:true,version:CORE_VERSION,legacy_version:'WA3-V1',actor:{id:a.actor_id,is_admin:a.is_admin===true,name:me.nombre||null},control:(Array.isArray(control.data)?control.data[0]:null)||{},boxes:boxRows,members:memberRows,users:userRows,transport:{kind:'sse-fetch',endpoint:'/api/wa3/events',fallback_after_ms:30000},canary:{enabled:String(WA_CANARY_MODE).toLowerCase()==='true',allowlist_count:String(WA_CANARY_ALLOW_TO).split(',').filter(Boolean).length}});
...
async function mutation(req,res,rpc,payload,adminOnly,eventType,conversationId){const a=await requireActor(req,res,!!adminOnly);if(!a)return null;payload=Object.assign({},payload||{}, {p_actor_id:a.actor_id});try{const out=await serviceRpc(rpc,payload);const d=out.data||{};const status=d.ok===false?409:200;if(status===200&&eventType)core.emit(eventType,{conversation_id:conversationId||(d&&d.conversation_id)||null,reason:rpc});writeJson(res,status,d);return d;}catch(e){console.error('[WA3] mutation',rpc,e.message);writeJson(res,503,{ok:false,error:'WA3_MUTATION_UNAVAILABLE'});return null;}}
async function boxUpsert(req,res,body){await mutation(req,res,'aos_wa3_box_upsert_v1',{p_box_id:body.box_id||null,p_code:String(body.code||''),p_name:String(body.name||''),p_strategy:String(body.strategy||'MANUAL').toUpperCase(),p_status:String(body.status||'ACTIVE').toUpperCase(),p_is_default:body.is_default===true,p_priority:Number.isFinite(Number(body.priority))?Number(body.priority):0},true,'configuration.changed');}
async function memberSet(req,res,body){if(!UUID_RE.test(String(body.box_id||''))||!UUID_RE.test(String(body.user_id||'')))return writeJson(res,400,{ok:false,error:'INVALID_MEMBER_INPUT'});await mutation(req,res,'aos_wa3_box_member_set_v1',{p_box_id:body.box_id,p_user_id:body.user_id,p_active:body.active!==false,p_max_active:body.max_active==null?null:Number(body.max_active),p_priority:Number.isFinite(Number(body.priority))?Number(body.priority):0},true,'configuration.changed');}
async function setControl(req,res,body){await mutation(req,res,'aos_wa3_admin_set_control_v1',{p_auto_routing_enabled:typeof body.auto_routing_enabled==='boolean'?body.auto_routing_enabled:null,p_human_send_enabled:typeof body.human_send_enabled==='boolean'?body.human_send_enabled:null},true,'configuration.changed');}
async function route(req,res,id,body){if(!UUID_RE.test(id)||!UUID_RE.test(String(body.box_id||'')))return writeJson(res,400,{ok:false,error:'INVALID_ROUTE_INPUT'});const owner=body.owner_user_id||null;if(owner&&!UUID_RE.test(String(owner)))return writeJson(res,400,{ok:false,error:'INVALID_OWNER'});await mutation(req,res,'aos_wa3_route_v1',{p_conversation_id:id,p_box_id:body.box_id,p_owner_user_id:owner,p_reason:String(body.reason||'ADMIN_ROUTE').slice(0,160)},true,'conversation.ownership',id);}
async function claimNext(req,res,body){if(!UUID_RE.test(String(body.box_id||'')))return writeJson(res,400,{ok:false,error:'INVALID_BOX_ID'});await mutation(req,res,'aos_wa3_claim_next_v1',{p_box_id:body.box_id},false,'conversation.ownership');}
async function release(req,res,id,body){if(!UUID_RE.test(id))return writeJson(res,400,{ok:false,error:'INVALID_CONVERSATION_ID'});await mutation(req,res,'aos_wa3_release_v1',{p_conversation_id:id,p_reason:String(body.reason||'HUMAN_RELEASE').slice(0,160)},false,'conversation.ownership',id);}
async function setMode(req,res,id,body){if(!UUID_RE.test(id))return writeJson(res,400,{ok:false,error:'INVALID_CONVERSATION_ID'});await mutation(req,res,'aos_wa3_set_mode_v1',{p_conversation_id:id,p_mode:String(body.mode||'').toUpperCase()},false,'conversation.ownership',id);}
...
function semanticBody(payload){return payload&&payload.type==='text'?String(payload.text&&payload.text.body||''):interactive.messageBody(payload);}
async function reserveOutbound(key,actorId,payload,msgType){const created=await sbRequest('POST','/rest/v1/aos_wa_outbound_requests_v1?on_conflict=idempotency_key',{idempotency_key:key,actor_id:actorId,to_number:payload.to,message_type:msgType,state:'PENDING',updated_at:new Date().toISOString()},true,'resolution=ignore-duplicates,return=representation');if(Array.isArray(created.data)&&created.data.length===1)return {owner:true,row:created.data[0]};const existing=await serviceGet('/rest/v1/aos_wa_outbound_requests_v1?idempotency_key=eq.'+encodeURIComponent(key)+'&select=idempotency_key,state,provider_message_id,error_code&limit=1');return {owner:false,row:Array.isArray(existing.data)?existing.data[0]||null:null};}
async function ownedSend(req,res,id,body){if(!UUID_RE.test(id))return writeJson(res,400,{ok:false,error:'INVALID_CONVERSATION_ID'});const token=strongToken(req);if(!token)return writeJson(res,403,{ok:false,error:'WA3_2FA_PANEL_REQUIRED'});if(!wa.validIdempotencyKey(body&&body.idempotency_key))return writeJson(res,400,{ok:false,error:'IDEMPOTENCY_KEY_REQUIRED'});if(!waConfigReadyOutbound())return writeJson(res,503,{ok:false,error:'WA_OUTBOUND_NOT_CONFIGURED'});
  ...
  const msgType=semanticType(payload),msgBody=semanticBody(payload);let reservation;try{reservation=await reserveOutbound(String(body.idempotency_key),auth.actor_id,payload,msgType);if(!reservation.owner){const row=reservation.row||{};return writeJson(res,row.state==='FAILED'?409:200,{ok:row.state!=='FAILED',idempotent:true,message_id:row.provider_message_id||null,status:row.state||'PENDING',error:row.state==='FAILED'?(row.error_code||'PREVIOUS_SEND_FAILED'):undefined});}
    ...
    await innerMetaRequest('POST','/api/wa/meta/reconcile-internal',{provider_message_id:String(messageId)},12000);
    core.emit('message.accepted',{conversation_id:id,reason:'human_send'});
...
function serveWa3Page(res){const file=path.join(__dirname,'public','admin-whatsapp-wa3.html');fs.stat(file,(err,st)=>{if(err||!st.isFile())return writeJson(res,503,{ok:false,error:'WA3_UI_UNAVAILABLE'});res.writeHead(200,{'Content-Type':'text/html; charset=utf-8','Cache-Control':'no-store','X-Content-Type-Options':'nosniff','X-Ascenda-WA3-Routing':'v1','X-Ascenda-Conversation-Core':CORE_VERSION});fs.createReadStream(file).pipe(res);});}
async function eventStream(req,res){const a=await requireActor(req,res,false);if(!a)return;try{core.subscribe(res,{id:a.actor_id,is_admin:a.is_admin===true});}catch(e){if(!res.headersSent)writeJson(res,e.status||503,{ok:false,error:e.message||'CONVERSATION_EVENT_STREAM_UNAVAILABLE'});}}
function proxy(req,res){
  const headers=Object.assign({},req.headers,{host:'127.0.0.1:'+INNER_PORT});
  let isWebhook=false;try{const u=new URL(req.url,'http://localhost');isWebhook=req.method==='POST'&&u.pathname==='/webhook';}catch(_){}
  const p=http.request({hostname:'127.0.0.1',port:INNER_PORT,path:req.url,method:req.method,headers},r=>{
    if(isWebhook&&Number(r.statusCode||500)>=200&&Number(r.statusCode||500)<300)r.on('end',()=>core.emit('provider.webhook',{reason:'meta_webhook'}));
    res.writeHead(r.statusCode||502,r.headers);r.pipe(res);
  });
  p.on('error',e=>{console.error('[WA3] proxy',e.message);if(!res.headersSent)writeJson(res,502,{ok:false,error:'WA3_INNER_UNAVAILABLE'});else res.end();});
  req.pipe(p);
}
...
const server=http.createServer(async(req,res)=>{let u;try{u=new URL(req.url,'http://localhost')}catch(e){return writeJson(res,400,{ok:false,error:'INVALID_URL'})}const p=u.pathname;if(req.method==='GET'&&p==='/admin-whatsapp.html'){serveWa3Page(res);return}if(p.startsWith('/api/wa3/')){if(!rateAllowed(req))return writeJson(res,429,{ok:false,error:'WA3_RATE_LIMIT'});try{
  ...
  if(req.method==='GET'&&p==='/api/wa3/events')return await eventStream(req,res);
...
function shutdown(sig){console.log('[WA3] shutdown',sig);try{core.close()}catch(e){}try{child.kill(sig)}catch(e){}server.close(()=>process.exit(0));setTimeout(()=>process.exit(1),5000).unref();}
