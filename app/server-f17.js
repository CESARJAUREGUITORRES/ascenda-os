...
function rpc(name, payload) {
  return new Promise(function(resolve, reject) {
    ...
    const req = https.request({
      ...
      res.on('end', function() {
        if ((res.statusCode || 500) < 200 || (res.statusCode || 500) >= 300) return reject(new Error('F17_AUTH_UNAVAILABLE'))
...
async function verifyApp(token) {
  ...
  try {
    const actorId = await rpc('aos_app_actor_v3', { p_token: t, p_required_panel: null, p_require_2fa: false })
...
function start() {
  ...
  server.listen(EXTERNAL_PORT, '0.0.0.0', function() {
    console.log('[F17] listening', { external: EXTERNAL_PORT, inner: INNER_PORT, gatewayConfigured: gateway.configured() })
  })
}

if (require.main === module) start()

module.exports = { verifyApp: verifyApp, gateway: gateway, server: server, start: start }
