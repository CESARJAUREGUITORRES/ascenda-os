// ASCENDA OS — K1 security front proxy (Phase 2 aware).
// Authority source: aos_app_sessions_v3 / aos_app_actor_v3.
// It does not create a second KronIA session system.
'use strict';
...
async function identity(req,requireAdmin){
  ...
  // additive runtime canary so the proxy can safely deploy before DB closure.
...
function proxy(req,res,body){
  const headers=Object.assign({},req.headers,{host:'127.0.0.1:'+PHASE2_PORT});delete headers['content-length'];
  if(body)headers['content-length']=Buffer.byteLength(body);
  const up=http.request({hostname:'127.0.0.1',port:PHASE2_PORT,path:req.url,method:req.method,headers},ur=>{res.writeHead(ur.statusCode||502,ur.headers);ur.pipe(res);});
...
const server=http.createServer(async(req,res)=>{
  ...
  if(pathname==='/api/kronia/chat'&&req.method==='POST'){
    ...
    const next=Buffer.from(JSON.stringify(d));req.headers.authorization='Bearer '+id.token;proxy(req,res,next);return;
  }
