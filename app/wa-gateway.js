  ...
  payload.entry.forEach(entry=>{
    (entry.changes||[]).forEach(change=>{
      ...
      (value.statuses||[]).forEach(st=>{
        ...
        statuses.push(row);
        events.push({event_key:'status:'+id+':'+state+':'+String(st.timestamp||''),event_type:'message.status',provider_message_id:id,status:state,payload:{recipient_id:row.recipient_id,recipient_user_id:row.recipient_user_id,recipient_parent_user_id:row.recipient_parent_user_id,recipient_kind:row.recipient_id?'PHONE':(row.recipient_user_id?'BSUID':'UNKNOWN'),pricing_category:row.pricing_category,pricing_model:row.pricing_model,pricing_type:row.pricing_type,billable:row.billable,error_code:row.error_code,provider_timestamp:row.provider_timestamp}});
