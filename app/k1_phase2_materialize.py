...
def once(s,old,new,label):
    ...
    if s.count(old)!=1: raise SystemExit(f'K1 materialize anchor mismatch {label}: {s.count(old)}')
...
# once in app.html covers Team/Config/Sales/Agents. Brain is standalone.
...
for p in ['public/app.html','public/cerebro.html']:
    ...
    if tag not in s:
        ...
        # preserve original body casing by replacing last literal occurrence
