...
var patched={adminLoad:null,advisorRender:null,advisorRead:null,adminRender:null};
...
function applyAdminLoadPatch(){
  if(typeof window.lNo!=='function'||window.lNo===patched.adminLoad)return;
  var fn=function(){sbRpc('aos_admin_notificaciones_v1',{p_limit:50}).then(function(d){if(window.D){D.no=d&&d.rows||[];if(D.rpTab==='n'&&typeof window.rNo==='function')window.rNo();}}).catch(function(){try{patched.adminLoad();}catch(_){}});};
  patched.adminLoad=window.lNo;window.lNo=fn;
}
function cardIcon(n){var m=meta(n&&n.channel);return m.emoji;}
function applyAdvisorRenderPatch(){
  if(typeof window.aNo!=='function'||window.aNo===patched.advisorRender)return;
  patched.advisorRender=window.aNo;
  window.aNo=function(){
    var el=document.getElementById('aNotifs');if(!el||!window.AD)return patched.advisorRender();
...
function applyAdminRenderPatch(){
  if(typeof window.rNo!=='function'||window.rNo===patched.adminRender)return;
  patched.adminRender=window.rNo;
  window.rNo=function(){
    var el=document.getElementById('zNL');if(!el||!window.D)return patched.adminRender();
...
function applyReadPatch(){
  if(typeof window.aRN!=='function'||window.aRN===patched.advisorRead)return;
  patched.advisorRead=window.aRN;
  window.aRN=function(id){var n=window.AD&&AD.no&&AD.no.find(function(x){return String(x.id)===String(id);});readAndOpen(id,n&&n.route,n&&n.entity_id);};
}
