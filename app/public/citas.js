...
var VC={mes:new Date().getMonth()+1, anio:new Date().getFullYear(), data:null, filtro:'all'};
...
function renderDetalle(rows, filtro){
  var filtered=filtro==='all'?rows:rows.filter(function(r){
    var e=(r.estado_cita||'').toUpperCase();
    var f=filtro.toUpperCase();
    if(f==='ASISTIO')return e.indexOf('ASISTI')>=0&&e.indexOf('NO')<0;
    if(f==='NO ASISTIO')return e.indexOf('NO ASISTI')>=0||e==='NO ASISTIO';
    return e===f;
  });
  var tb=el('vc-tbody');
  ...
  tb.innerHTML=filtered.map(function(c){
    ...
    return '<tr><td style="white-space:nowrap;color:#6B7BA8;">'+h(c.fecha_cita||'')+'</td>'+
...
function renderEstados(estados){
  var colors={'PENDIENTE':'#D97706','ASISTIO':'#16A34A','NO ASISTIO':'#DC2626','CANCELADA':'#6B7BA8'};
