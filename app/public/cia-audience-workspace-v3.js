...
function releaseCall(){var a=state.release&&Array.isArray(state.release.channels)?state.release.channels:[];return a.find(function(x){return x.channel==='CALL'})||{stage:'HUMAN_CANARY_REQUIRED',execution_enabled:false,max_source_limit:1,max_advisors:1}}
function plannerTargetPayload(){
...
function loadRelease(){return rpc('DISTRIBUTION_RELEASE_STATE',{}, {key:'release'}).then(function(d){state.release=d;if(state.tab==='distribution')renderDistribution();return d}).catch(function(e){if(!silent(e))console.warn('[CIA-V4] release-state load failed',e);return null})}
function open(){
  ...
  loadBoot();loadLibrary();loadRelease();
}
