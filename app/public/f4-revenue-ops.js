...
function loadCandidates(caseRow){
  ...
  var base='https://ituyqwstonmhnfshnaqz.supabase.co/rest/v1/rpc/aos_cartera_candidates_v2';var headers={'Content-Type':'application/json'};
  ...
  nativeFetch(base,{method:'POST',headers:headers,body:JSON.stringify({p_token:token(),p_case_id:caseRow.id}),cache:'no-store'})
    .then(function(r){return r.json()})
    .then(function(d){
      var list=document.getElementById('f4-car-list');if(!list)return;
      if(!d||!d.ok){list.textContent='No fue posible consultar coincidencias.';return}
      var cs=d.candidates||[];
      if(!cs.length){list.innerHTML='<span style="font-size:9px;color:#8291B3">Sin coincidencias suficientemente fuertes. Puedes clasificar manualmente el caso, excepto PAGO_RECONCILIADO que exige evidencia vinculada.</span>';return}
      list.innerHTML=cs.map(function(c,i){return '<button type="button" data-f4cand="'+i+'" style="display:block;width:100%;text-align:left;border:1px solid #DDE4F5;background:#fff;border-radius:8px;padding:8px;margin:5px 0;cursor:pointer"><b>'+esc(c.type)+' · '+esc(c.date||'')+' · '+money(c.totalAmount)+'</b><br><span style="font-size:9px;color:#6B7BA8">Score '+esc(c.score)+' · '+esc(c.confidence)+' · '+esc(candidateSummary(c))+'</span></button>'}).join('');
      Array.prototype.forEach.call(list.querySelectorAll('[data-f4cand]'),function(b){
        b.onclick=function(){
          ...
          if(c.type==='COTIZACION'){if(total&&c.totalAmount!=null)total.value=c.totalAmount;if(bal&&c.balanceAmount!=null)bal.value=c.balanceAmount}
          var note=document.getElementById('car-edit-note');if(note&&!note.value)note.value='F4: evidencia vinculada '+c.type+' '+c.id;
        };
      });
    })
    .catch(function(){var list=document.getElementById('f4-car-list');if(list)list.textContent='Error consultando coincidencias.'});
}
