...
function cc6NoShowGate(prep,done){var p=cc6Policy(prep),ns=p.lastNoShow;if(!ns){done();return;}var current=String(prep.asesor||'').toUpperCase(),owner=String(ns.advisor||'').toUpperCase(),same=current&&owner&&current===owner,until=new Date(ns.protectedUntil||0),now=new Date(),protectedNow=until&&!isNaN(until)&&now<until,follow=!!ns.ownerFollowupAfterNoShow,title='Oportunidad con NO ASISTIO',body='';if(same){body='<b>Esta oportunidad ya era tuya.</b><br>Reagendarla conserva tu propiedad, pero no genera una segunda cita comercial por la misma oportunidad.';}else if(protectedNow){body='<b>Propietario actual: '+cc6Esc(owner)+'</b><br>La ventana de protección vence '+cc6Esc(cc6Local(ns.protectedUntil))+'.<br>Tu llamada quedará registrada como ayuda/seguimiento y la nueva Agenda seguirá a nombre de '+cc6Esc(owner)+'. No se generará una segunda cita KPI.';}else if(follow){body='<b>'+cc6Esc(owner)+' ya registró seguimiento.</b><br>La oportunidad continúa bajo su propiedad. Tu gestión puede ayudar a reagendar, pero no transfiere el crédito comercial.';}else{body='<b>Recuperación disponible.</b><br>Ya pasaron 72 horas y no existe seguimiento registrado del propietario anterior ('+cc6Esc(owner)+'). Si realmente consigues una nueva cita, el servidor podrá transferir la oportunidad y acreditar la nueva llamada+cita a tu usuario.';}cc6InfoModal('cc6-noshow-modal',title,'<div style="padding:10px;background:#EFF6FF;border:1px solid #BFDBFE;border-radius:9px">NO ASISTIO: '+cc6Esc(ns.date||'')+' '+cc6Esc(ns.time||'')+'<br>'+body+'</div>','Entendido, continuar',done);}
