/* ASCENDA OS · MKT-INTEGRITY-HOTFIX-V3 · LOOP 6 CREDIT/OWNERSHIP V2
 * Loaded after calls.html legacy runtime. Server policy is authoritative.
 */
(function(){
'use strict';
...
function cc6ClearPending(fp){try{var old=JSON.parse(sessionStorage.getItem('aos_cc6_pending_action')||'null');if(!fp||!old||old.fp===fp)sessionStorage.removeItem('aos_cc6_pending_action');}catch(_e){try{sessionStorage.removeItem('aos_cc6_pending_action');}catch(_e2){}}}
function cc6Toast(t,s,cls){if(window.AOS_showToast)AOS_showToast(t,s||'',cls||'');}
function cc6SetBusy(v){if(!window.CC)return;CC.guardando=!!v;['#cc-m-cita .mconf.gr','#cc-m-cita-manual .mconf.gr'].forEach(function(sel){var b=document.querySelector(sel);if(b)b.disabled=!!v;});}
function cc6Esc(s){return typeof window.escH==='function'?window.escH(s):String(s||'').replace(/[&<>"']/g,function(c){return{'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot',"'":'&#39;'}[c];});}
function cc6TransientPrepareError(e){var p=e&&e.payload||{},s=String((p&&(p.error||p.message||p.code))||(e&&e.message)||'').toUpperCase();return Number(e&&e.status)===500||Number(e&&e.status)===503||s.indexOf('57014')>=0||s.indexOf('STATEMENT TIMEOUT')>=0||s.indexOf('HTTP 500')>=0||s.indexOf('HTTP 503')>=0||s.indexOf('FAILED TO FETCH')>=0||s.indexOf('ABORT')>=0;}
function cc6Prepare(num,attempt){var a=Number(attempt)||0,p={p_token:cc6Token(),p_numero:String(num||'').replace(/\D/g,'')};return cc6Rpc('aos_callcenter_prepare_action_v1',p).catch(function(e){if(a>=2||!cc6TransientPrepareError(e))throw e;var wait=a===0?450:1100;console.warn('[CC6V23] prepare transient retry',a+1,e&&e.message);return new Promise(function(resolve){setTimeout(resolve,wait);}).then(function(){return cc6Prepare(num,a+1);});});}
function cc6Policy(prep){return(prep&&prep.creditPolicy)||{};}
function cc6Local(iso){if(!iso)return'—';try{return new Intl.DateTimeFormat('es-PE',{timeZone:'America/Lima',day:'2-digit',month:'2-digit',year:'numeric',hour:'2-digit',minute:'2-digit'}).format(new Date(iso));}catch(_e){return String(iso);}}
function cc6EvidenceLabel(prep){var e=(prep&&prep.evidence)||{},out=[];if(e.priorSale)out.push('venta previa'+(e.lastSale&&e.lastSale.fecha?' '+e.lastSale.fecha:''));if(e.priorAttention)out.push('atención previa'+(e.lastAttention&&e.lastAttention.fecha?' '+e.lastAttention.fecha:''));if(e.priorAttendedAppointment)out.push('cita asistida/efectiva'+(e.lastAttendedAppointment&&e.lastAttendedAppointment.fecha?' '+e.lastAttendedAppointment.fecha:''));return out.length?out.join(' · '):'sin evidencia fuerte previa';}
function cc6RemoveModal(id){var x=document.getElementById(id);if(x)x.remove();}
function cc6InfoModal(id,title,html,button,done){cc6RemoveModal(id);var m=document.createElement('div');m.id=id;m.className='mov open';m.style.zIndex='1250';m.innerHTML='<div class="modal" style="max-width:520px"><div class="mhd"><span style="font-size:22px">⚠️</span><div><div class="mtit">'+cc6Esc(title)+'</div></div></div><div style="font-size:11px;line-height:1.55;color:#34425E">'+html+'</div><div class="mfoot"><button class="mcanc" data-cancel>Cancelar</button>'+(button?'<button class="mconf gr" data-ok>'+cc6Esc(button)+'</button>':'')+'</div></div>';document.body.appendChild(m);var c=m.querySelector('[data-cancel]');if(c)c.onclick=function(){m.remove();cc6SetBusy(false);};var ok=m.querySelector('[data-ok]');if(ok)ok.onclick=function(){m.remove();if(done)done();};}

/* Capture Contact Debt metadata. */
if(typeof window._rpc==='function'&&!window._rpc.__cc6wrapped){var _rpc0=window._rpc;function wrappedRpc(fn,p,ok,fail){return _rpc0(fn,p,function(res){if(ok)ok(res);if(fn==='aos_siguiente_lead'&&res&&res.ok&&res.lead&&window.CC&&CC.lead&&String(CC.lead.num||'')===String(res.lead.num||'')){CC.lead.leadId=res.lead.id!=null?Number(res.lead.id):null;CC.lead.anuncio=res.lead.anuncio||((res.anuncio&&res.anuncio.nombre)||'');CC.lead.horaIngreso=res.lead.hora_ingreso||null;CC.lead.attributionSource=res.lead.attributionSource||'UNRESOLVED';CC.lead.contactDebtBucket=res.lead.contactDebtBucket||res.contactDebtBucket||null;CC.lead.waitMinutes=res.lead.waitMinutes||res.waitMinutes||0;setTimeout(function(){cc6ClassifyCurrentLead();},0);}},fail);}wrappedRpc.__cc6wrapped=true;window._rpc=wrappedRpc;}

function cc6ClassifyCurrentLead(){if(!window.CC||!CC.lead||!CC.lead.num)return;var num=CC.lead.num;cc6Prepare(num).then(function(prep){if(!CC.lead||String(CC.lead.num)!==String(num))return;CC.loop6Patient=prep;CC.loop6Blocked=!!(prep&&prep.error==='IDENTITY_CONFLICT');var tier=document.getElementById('cc-tier'),ctx=document.getElementById('cc-contexto'),p=cc6Policy(prep);if(CC.loop6Blocked){if(tier)tier.textContent='REVIEW · IDENTIDAD';if(ctx){ctx.style.display='block';ctx.innerHTML='<div class="ctx-bloque" style="border-left-color:#DC2626;background:#FEF2F2"><div class="ctx-tit" style="color:#DC2626">⚠️ Identidad ambigua</div><div class="ctx-row"><span class="ctx-val">No registrar conversión automática. Revisar identidad.</span></div></div>';}return;}if(prep.patientState==='CONVERTED_PATIENT'){if(tier)tier.textContent='PACIENTE EXISTENTE';if(ctx){var current=ctx.innerHTML||'',elig=p.reactivationEligible?'Reactivación elegible':'Reactivación elegible desde '+cc6Local(p.reactivationEligibleFrom);var note='<div class="ctx-bloque" id="cc6-patient-context" style="border-left-color:#D97706;background:#FFF7ED"><div class="ctx-tit" style="color:#D97706">♻️ Paciente convertido</div><div class="ctx-row"><span class="ctx-val">'+cc6Esc(cc6EvidenceLabel(prep))+' · '+cc6Esc(elig)+'</span></div></div>';if(current.indexOf('cc6-patient-context')<0){ctx.style.display='block';ctx.innerHTML=current+note;}}}else if(p.lastNoShow&&ctx){var ns=p.lastNoShow,free=ns.protectedUntil&&new Date(ns.protectedUntil)<new Date(),txt='NO ASISTIO '+ns.date+' '+(ns.time||'')+' · '+(free?'Propiedad liberada':'Propietario temporal: '+(ns.advisor||'—')+' · protección hasta '+cc6Local(ns.protectedUntil));var cur=ctx.innerHTML||'';if(cur.indexOf('cc6-noshow-context')<0){ctx.style.display='block';ctx.innerHTML=cur+'<div class="ctx-bloque" id="cc6-noshow-context" style="border-left-color:#2563EB;background:#EFF6FF"><div class="ctx-tit" style="color:#2563EB">↩️ Oportunidad con NO ASISTIO</div><div class="ctx-row"><span class="ctx-val">'+cc6Esc(txt)+'</span></div></div>';}}}).catch(function(e){console.warn('[CC6V2] patient state unavailable',e);});}
...
function cc6ActiveBlock(prep){var a=cc6Policy(prep).activeAppointment;if(!a)return false;cc6InfoModal('cc6-active-modal','Ya existe una cita activa','<div style="padding:10px;background:#FEF2F2;border:1px solid #FECACA;border-radius:9px"><b>Asesor:</b> '+cc6Esc(a.advisor||'—')+'<br><b>Fecha:</b> '+cc6Esc(a.date||'—')+' · '+cc6Esc(a.time||'—')+'<br><b>Estado:</b> '+cc6Esc(a.status||'—')+'</div><p>No se creará una segunda conversión. Si necesita reprogramarse, debe tratarse como continuidad/reagendamiento.</p>',null);return true;}
function cc6NoShowGate(prep,done){
  var p=cc6Policy(prep),ns=p.lastNoShow;if(!ns){done();return;}
  var current=String(prep.asesor||'').toUpperCase(),owner=String(ns.advisor||'').toUpperCase(),
      same=current&&owner&&current===owner,until=new Date(ns.protectedUntil||0),now=new Date(),
      protectedNow=until&&!isNaN(until)&&now<until,follow=!!ns.ownerFollowupAfterNoShow,
      last=ns.ownerLastActivityAt?cc6Local(ns.ownerLastActivityAt):null,
      title='Oportunidad con NO ASISTIO',body='';
  if(same){
    body='<b>Esta oportunidad ya era tuya.</b><br>Reagendarla conserva tu propiedad, pero no genera una segunda cita comercial por la misma oportunidad.';
    cc6InfoModal('cc6-noshow-modal',title,'<div style="padding:10px;background:#EFF6FF;border:1px solid #BFDBFE;border-radius:9px">NO ASISTIO: '+cc6Esc(ns.date||'')+' '+cc6Esc(ns.time||'')+'<br>'+body+'</div>','Entendido, continuar',done);
    return;
  }
  if(protectedNow||follow){
    body='<b>Propietario temporal: '+cc6Esc(owner)+'</b><br>La protección vence '+cc6Esc(cc6Local(ns.protectedUntil))+'.'+(last?'<br>Última gestión del propietario: '+cc6Esc(last)+'.':'')+'<br>Mientras la ventana siga activa, tu gestión puede ayudar pero no transfiere el crédito comercial.';
    cc6InfoModal('cc6-noshow-modal',title,'<div style="padding:10px;background:#EFF6FF;border:1px solid #BFDBFE;border-radius:9px">NO ASISTIO: '+cc6Esc(ns.date||'')+' '+cc6Esc(ns.time||'')+'<br>'+body+'</div>','Entendido, continuar',done);
    return;
  }
  cc6Toast('♻️ Recuperación disponible','Pasaron 72 h sin una nueva gestión del propietario anterior. Si consigues una cita, la oportunidad puede transferirse a tu usuario.','');
  done();
}
function cc6ExistingPatientModal(prep,payload,done){cc6RemoveModal('cc6-existing-patient-modal');var p=cc6Policy(prep),eligible=!!p.reactivationEligible,eligTxt=eligible?'<b style="color:#15803D">✓ Reactivación comercial elegible.</b>':'<b style="color:#B45309">Aún no cumple 15 días.</b> Elegible desde '+cc6Esc(cc6Local(p.reactivationEligibleFrom))+'. Si eliges Reactivación ahora, se guardará gestión + Agenda pero no una nueva cita comercial; beneficiario: Clínica.';var m=document.createElement('div');m.id='cc6-existing-patient-modal';m.className='mov open';m.style.zIndex='1200';var name=((payload.nombre||'')+' '+(payload.apellido||'')).trim()||payload.numero;m.innerHTML='<div class="modal" style="max-width:520px"><div class="mhd"><span style="font-size:22px">⚠️</span><div><div class="mtit">PACIENTE EXISTENTE DETECTADO</div><div style="font-size:10px;color:#9AAAC8">'+cc6Esc(name)+' · '+cc6Esc(payload.numero)+'</div></div></div><div style="padding:9px;background:#FFF7ED;border:1px solid #FED7AA;border-radius:9px;font-size:10px;color:#92400E;line-height:1.5;margin-bottom:10px"><b>Evidencia:</b> '+cc6Esc(cc6EvidenceLabel(prep))+'<br>'+eligTxt+'</div><div style="display:grid;gap:7px"><button data-action="REACTIVATION" style="padding:11px;border-radius:9px;border:1px solid #BBF7D0;background:#F0FDF4;text-align:left;cursor:pointer"><b>♻️ Reactivación comercial</b><br><span style="font-size:9px;color:#6B7BA8">'+(eligible?'Genera llamada+cita comercial sin nueva adquisición.':'Se registra como seguimiento no elegible hasta cumplir 15 días.')+'</span></button><button data-action="PATIENT_FOLLOWUP" style="padding:11px;border-radius:9px;border:1px solid #BFDBFE;background:#EBF2FF;text-align:left;cursor:pointer"><b>📞 Seguimiento de paciente</b><br><span style="font-size:9px;color:#6B7BA8">Gestión real sin nueva conversión.</span></button><button data-action="AGENDA_ONLY" style="padding:11px;border-radius:9px;border:1px solid #E2E8F0;background:#F8FAFC;text-align:left;cursor:pointer"><b>📅 Solo agendar cita</b><br><span style="font-size:9px;color:#6B7BA8">Agenda únicamente; no suma llamada/cita comercial.</span></button></div><div class="mfoot"><button class="mcanc" id="cc6-existing-cancel">Cancelar</button></div></div>';document.body.appendChild(m);m.querySelectorAll('[data-action]').forEach(function(b){b.onclick=function(){var a=this.getAttribute('data-action');m.remove();done(a);};});document.getElementById('cc6-existing-cancel').onclick=function(){m.remove();cc6SetBusy(false);};}
function cc6ManualChoice(payload,done){var mode=((document.getElementById('cc6-manual-mode')||{}).value||'COMMERCIAL');if(mode==='AGENDA_ONLY'){done('AGENDA_ONLY');return;}done(mode==='CALLBACK'?'CALLBACK_INBOUND_APPOINTMENT':'COMMERCIAL_CALL_APPOINTMENT');}
function cc6PostCommitAppointment(res,payload){
  var agendaId=String(res&&res.agendaId||res&&res.agenda_id||'').trim();
  if(agendaId&&typeof window.aosQueueGoogleAppointment==='function'){
    Promise.resolve(window.aosQueueGoogleAppointment(agendaId)).then(function(g){
      if(g&&g.ok===false)console.warn('[CC6V23] Google queue pendiente',agendaId,g);
    }).catch(function(e){console.warn('[CC6V23] Google queue fail-open',agendaId,e&&e.message);});
  }
  if(payload.correo&&typeof window.enviarEmailConfirmacionCita==='function'){
    window.enviarEmailConfirmacionCita({
      id:agendaId,
...
cc6EnsureSelectors();setTimeout(function(){if(typeof window.loadLead==='function'&&(!window.CC||!CC.lead||!CC.lead.num))loadLead();},80);console.log('[ASCENDA][CC6V2] credit/ownership policy ready');
})();