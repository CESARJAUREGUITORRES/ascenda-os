...
function aosQueueGoogleAppointment(id){
...
function _rpc(fn,p,ok,fail){
  ...
  .then(function(r){
    ...
    return r.text().then(function(raw){
      var d=null;
      if(raw){try{d=JSON.parse(raw);}catch(_e){d=null;}}
      if(!r.ok){
        var msg=(d&&(d.error||d.message||d.code))||('HTTP '+r.status);
        var err=new Error(String(msg));
        err.status=r.status;err.payload=d||{};err.responseText=raw||'';
        console.error('[SB RPC]',fn,'HTTP '+r.status,d||raw||'');
        throw err;
      }
      return d;
    });
