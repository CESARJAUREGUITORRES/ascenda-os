/**
 * ╔══════════════════════════════════════════════════════════════╗
 * ║  AscendaOS v1 — GS_13_Marketing.gs v4.0                    ║
 * ║  CORRECCIÓN DEFINITIVA: usa da_* del GS_04_DataAccess       ║
 * ║                                                             ║
 * ║  Los da_* ya tienen los índices correctos de GS_01_Config:  ║
 * ║  LEADS:   NUM_LIMPIO = col H (índice 7) via LEAD_COL        ║
 * ║  LLAM:    NUM_LIMPIO = col I (índice 8) via LLAM_COL        ║
 * ║  VENTAS:  NUM_LIMPIO = col P (índice 15) via VENT_COL       ║
 * ║  AGENDA:  NUMERO = col F (índice 5) via AG_COL              ║
 * ║  INVERSION: via da_inversionData(mes, anio, 'mes')          ║
 * ╚══════════════════════════════════════════════════════════════╝
 */

// ══════════════════════════════════════════════════════════════
// DASHBOARD MARKETING — función principal
// ══════════════════════════════════════════════════════════════
...
function api_getMarketingDashboard(mes, anio) {
  ...
  // Rango del mes seleccionado (primer y último día)
  var desde = new Date(anio, mes - 1, 1, 0, 0, 0);
  var hasta = new Date(anio, mes, 0, 23, 59, 59);

  // ── Leer con da_* (usan los índices correctos de GS_01_Config) ──
  ...
  // Leer agenda directamente (no existe da_agendaData, usamos _shAgenda)
  var shAg = _shAgenda();
  ...
  // ── Índice de leads por NUM_LIMPIO (llave universal) ──
  var leadNums = {}; // set de números únicos de leads del período
  ...
  // ── LLAMADOS: leads únicos que tienen al menos 1 llamada en el período ──
  var llamadosNums  = {};
  var tipifCount    = {};
  var intentosMap   = {};
  var horaCount     = {};
  ...
  llam.forEach(function(l) {
    ...
    // Conteo de tipificaciones (todas las llamadas, no solo leads)
    var t = l.estado || 'SIN TIPIF';
    tipifCount[t] = (tipifCount[t] || 0) + 1;

    // Solo contar como llamado si el número es un lead del período
    if (leadNums[k]) llamadosNums[k] = true;

    // Intentos por lead del período
    if (leadNums[k]) intentosMap[k] = (intentosMap[k] || 0) + 1;
  ...
  // 1er contacto: días entre fecha del lead y primera llamada
  leads.forEach(function(l) {
    var k = l.num;
    var primLlam = null;
  ...
  // ── CITAS TELEFÓNICAS: leads con estado CITA CONFIRMADA en llamadas ──
  var citasNums = {};
  llam.forEach(function(l) {
    if (l.estado === 'CITA CONFIRMADA' && leadNums[l.num]) citasNums[l.num] = true;
  });
  var nCitas = Object.keys(citasNums).length;

  // ── ASISTIERON: leads con cita en AGENDA con estado ASISTIO o EFECTIVA ──
  var asistNums = {};
  ...
  // ── VENTAS: leads con venta en el período ──
  var ventasNums = {};
  ventas.forEach(function(v) {
    if (leadNums[v.num]) ventasNums[v.num] = true;
  });
  var nVentas   = Object.keys(ventasNums).length;
  var factTotal = ventas
    .filter(function(v) { return leadNums[v.num]; })
    .reduce(function(s, v) { return s + v.monto; }, 0);

  // ── INVERSIÓN TOTAL del mes ──
  var invTotal = Object.values(invMap).reduce(function(s, v) { return s + v; }, 0);
  ...
  var llamTot   = llam.length; // total de llamadas del período (no solo leads)
...
function _buildAnuncioStats(leads, llam, ag, ventas, invTotal, nLeads) {
  ...
  llam.forEach(function(l) {
    var an = (l.num && numToAn[l.num]) ? numToAn[l.num] : null;
    if (!an || !leadNums[l.num]) return;
    add(an, 'llamadas', 1);
    if (l.estado === 'CITA CONFIRMADA') add(an, 'citas', 1);
  });
  ...
  return Object.values(map).sort(function(a, b) { return b.leads - a.leads; }).slice(0, 12).map(function(d) {
...
function _buildTratStats(leads, llam, ag, ventas, invMap, invTotal, nLeads) {
  ...
  llam.forEach(function(l) {
    if (!l.num) return;
    var t = l.trat || (numToTrat[l.num]) || '(sin trat.)';
    add(t, 'llamadas', 1);
    if (l.estado === 'CITA CONFIRMADA') add(t, 'citas', 1);
  });
  ...
  ventas.forEach(function(v) {
    var t = v.trat || (numToTrat[v.num]) || '(sin trat.)';
    add(t, 'ventas', 1);
    add(t, 'fact', v.monto);
  });

  return Object.values(map).sort(function(a, b) { return b.leads - a.leads; }).slice(0, 15).map(function(d) {
    // Inversión: usar la hoja si tiene el tratamiento, sino proporcional
    var invTrat = invMap[d.nombre] || (invTotal > 0 && nLeads > 0 ? +(invTotal * d.leads / nLeads).toFixed(2) : 0);
...
function _buildHistorialMeses(anio, mesActual, n) {
  ...
  for (var i = 0; i < n; i++) {
    ...
    var a = anio;
    if (m <= 0) { m += 12; a--; }
    var desde = new Date(a, m - 1, 1, 0, 0, 0);
    var hasta = new Date(a, m,     0, 23, 59, 59);

    var llds = da_leadsData(desde, hasta);
    var llms = da_llamadasData(desde, hasta);
    var vcs  = da_ventasData(desde, hasta);

    // Agenda del mes para contar citas
    var shAg = _shAgenda();
    var lrAg = shAg.getLastRow();
    var nC = 0;
    if (lrAg >= 2) {
      shAg.getRange(2, 1, lrAg - 1, 14).getValues().forEach(function(r) {
        if (_inRango(r[AG_COL.FECHA], desde, hasta) && _up(r[AG_COL.ESTADO]) !== 'CANCELADA') nC++;
      });
    }

    var fV = vcs.reduce(function(s, v) { return s + v.monto; }, 0);
    hist.push({
      mes: m, anio: a,
      mesNom: MESES_ES[m].slice(0, 3),
      leads:   llds.length,
      llamados:llms.length,
      citas:   nC,
      ventas:  vcs.length,
      fact:    fV,
...
function api_getAdminCallsPanel(mes, anio, asesor) {
  ...
  items.forEach(function(x) {
    var a = x.asesor || 'SIN ASESOR';
    if (!porAsesor[a]) porAsesor[a] = { total: 0, citas: 0, nc: 0 };
    porAsesor[a].total++;
    if (x.estado === 'CITA CONFIRMADA') porAsesor[a].citas++;
    if (x.estado === 'NO CONTESTA')     porAsesor[a].nc++;
  });
...
function test_Marketing() {
  Logger.log('=== GS_13_Marketing v4.0 TEST ===');
  Logger.log('Usa da_leadsData / da_llamadasData / da_ventasData / da_inversionData');
  Logger.log('Índices tomados de GS_01_Config (LEAD_COL, LLAM_COL, VENT_COL, AG_COL)');
  Logger.log('=== OK ===');
}