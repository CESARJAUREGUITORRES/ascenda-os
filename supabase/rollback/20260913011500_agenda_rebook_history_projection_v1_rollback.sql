-- Rollback Agenda rebook history projection V1.
-- Restores the certified V3 bridge and removes only the read-only history projection.

drop function if exists public.aos_agenda_rebook_history_day_v1(text,date,text);
drop table if exists public.aos_agenda_rebook_history_v1;
