...
-- Versioned deterministic late-lead trace-link backfill.
-- Idempotent: every row is guarded by NULL trace-link preconditions.
-- This file is NOT an automatic migration. Apply only under the active MKT-INTEGRITY-HOTFIX-V3 lock.

begin;

update public.aos_llamadas ll
set lead_id_origen = x.lead_id
from (values
...
) x(id,lead_id,call_id)
where a.id=x.id
  and a.lead_id_origen is null
  and a.llamada_id_origen is null;
