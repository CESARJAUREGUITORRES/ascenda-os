...
select
  ...
  e.value_tier,e.lifecycle,e.engagement,e.followup_pending,e.followup_overdue,e.next_followup_at,
  null::text latest_call_status,null::timestamptz last_call_at,
...
begin
  ...
  if not found then return jsonb_build_object('ok',false,'error','WORK_ITEM_NOT_OWNED'); end if;
  select upper(nullif(btrim(c.estado),'')),c.created_at
    into v_call_status,v_call_at
  from public.aos_llamadas c
  where public.aos_cia_normalize_contact_key_v1(c.numero_limpio)=v_row.contact_key
  order by c.created_at desc nulls last,c.fecha desc,c.id desc
  limit 1;
  v_item:=to_jsonb(v_row) || jsonb_build_object('latest_call_status',v_call_status,'last_call_at',v_call_at);
