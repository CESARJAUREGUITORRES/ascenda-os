...
as $$
declare
  v_state text:=pg_catalog.upper(pg_catalog.coalesce(p_state,'UNKNOWN'));
  v_severity text:=pg_catalog.upper(pg_catalog.coalesce(p_severity,'MEDIUM'));
  v_core jsonb;
  v_digest text;
begin
  ...
  if pg_catalog.jsonb_typeof(pg_catalog.coalesce(p_expected,'{}'::jsonb)) <> 'object' or pg_catalog.jsonb_typeof(pg_catalog.coalesce(p_actual,'{}'::jsonb)) <> 'object' or pg_catalog.jsonb_typeof(pg_catalog.coalesce(p_limitations,'[]'::jsonb)) <> 'array' then raise exception 'F6_6_INVALID_ENVELOPE_JSON'; end if;
  v_core:=pg_catalog.jsonb_build_object('signal_id',p_signal_id,'signal_name',p_signal_name,'owning_workstream',p_owner,'affected_contract',p_contract,'state',v_state,'severity',v_severity,'expected',pg_catalog.coalesce(p_expected,'{}'::jsonb),'actual',pg_catalog.coalesce(p_actual,'{}'::jsonb),'source_version',p_source_version,'baseline_version',p_baseline_version,'runbook_ref',p_runbook_ref,'limitations',pg_catalog.coalesce(p_limitations,'[]'::jsonb));
...
begin
  ...
  if not (p_snapshot ? 'f5_source') or pg_catalog.coalesce((p_snapshot#>>'{f5_source,batches_total}')::integer,0)=0 then v_state:='UNKNOWN'; elsif pg_catalog.coalesce((p_snapshot#>>'{f5_source,staging_complete_batches}')::integer,-1)<>pg_catalog.coalesce((p_snapshot#>>'{f5_source,batches_total}')::integer,0) or pg_catalog.coalesce((p_snapshot#>>'{f5_source,matched_batches}')::integer,-1)<>pg_catalog.coalesce((p_snapshot#>>'{f5_source,batches_total}')::integer,0) or pg_catalog.coalesce((p_snapshot#>>'{f5_source,expected_rows}')::bigint,-1)<>pg_catalog.coalesce((p_snapshot#>>'{f5_source,persisted_rows}')::bigint,-2) or pg_catalog.coalesce((p_snapshot#>>'{f5_source,mismatched_batches}')::integer,0)>0 or pg_catalog.coalesce((p_snapshot#>>'{f5_source,continuity_failures}')::integer,0)>0 then v_state:='BROKEN'; else v_state:='OK'; end if;
  v_signal:=public.aos_sentinel_rev_f6_6_signal_envelope_v1('SEN-DQ-F5-001','SOURCE_BATCH_MISMATCH','REV-F5','REV-F5_SOURCE_TRUTH',v_state,'HIGH',v_b->'f5_source',pg_catalog.coalesce(p_snapshot->'f5_source','{}'::jsonb),'REV-F5-CERTIFIED','1','docs/control/SENTINEL_DATA_INTEGRITY_SIGNALS_CONTRACT.md#sen-dq-f5-001-source_batch_mismatch'); v_signals:=v_signals||pg_catalog.jsonb_build_array(v_signal);
  if not (p_snapshot ? 'f5_membership') then v_state:='UNKNOWN'; elsif pg_catalog.coalesce((p_snapshot#>>'{f5_membership,source_rows}')::bigint,-1)<>pg_catalog.coalesce((p_snapshot#>>'{f5_membership,members}')::bigint,-2) or pg_catalog.coalesce((p_snapshot#>>'{f5_membership,orphans}')::bigint,0)>0 or pg_catalog.coalesce((p_snapshot#>>'{f5_membership,invalid_multiplicity}')::bigint,0)>0 then v_state:='BROKEN'; else v_state:='OK'; end if;
  v_signal:=public.aos_sentinel_rev_f6_6_signal_envelope_v1('SEN-DQ-F5-002','IDENTITY_MEMBERSHIP_MISMATCH','REV-F5','REV-F5_IDENTITY_MEMBERSHIP',v_state,'CRITICAL',v_b->'f5_membership',pg_catalog.coalesce(p_snapshot->'f5_membership','{}'::jsonb),'REV-F5-CERTIFIED','1','docs/control/SENTINEL_DATA_INTEGRITY_SIGNALS_CONTRACT.md#sen-dq-f5-002-identity_membership_mismatch'); v_signals:=v_signals||pg_catalog.jsonb_build_array(v_signal);
  if not (p_snapshot ? 'identity_bridge') then v_state:='UNKNOWN'; elsif pg_catalog.coalesce((p_snapshot#>>'{identity_bridge,resolved_collisions}')::bigint,0)>0 or pg_catalog.coalesce((p_snapshot#>>'{identity_bridge,resolved_bad_candidate_count}')::bigint,0)>0 or pg_catalog.coalesce((p_snapshot#>>'{identity_bridge,resolved_missing_target}')::bigint,0)>0 then v_state:='BROKEN'; else v_state:='OK'; end if;
  v_signal:=public.aos_sentinel_rev_f6_6_signal_envelope_v1('SEN-DQ-F5-003','IDENTITY_BRIDGE_COLLISION','REV-F5','REV_PATIENT_IDENTITY_BRIDGE_V2',v_state,'CRITICAL',pg_catalog.jsonb_build_object('resolved_collisions',0,'resolved_bad_candidate_count',0,'resolved_missing_target',0),pg_catalog.coalesce(p_snapshot->'identity_bridge','{}'::jsonb),'REV-F6.3','1','docs/control/SENTINEL_DATA_INTEGRITY_SIGNALS_CONTRACT.md#sen-dq-f5-003-identity_bridge_collision',pg_catalog.jsonb_build_array('KNOWN_CONFLICT_KEYS_ARE_FAIL_CLOSED_REVIEW_INVENTORY_NOT_RESOLVED_COLLISIONS')); v_signals:=v_signals||pg_catalog.jsonb_build_array(v_signal);
  if not (p_snapshot ? 'f5_apply') then v_state:='UNKNOWN'; elsif pg_catalog.coalesce((p_snapshot#>>'{f5_apply,missing_governance}')::bigint,0)>0 then v_state:='BROKEN'; else v_state:='OK'; end if;
  v_signal:=public.aos_sentinel_rev_f6_6_signal_envelope_v1('SEN-DQ-F5-004','APPLY_WITHOUT_GOVERNANCE','REV-F5','REV-F5_GOVERNED_APPLY',v_state,'CRITICAL',pg_catalog.jsonb_build_object('missing_governance',0),pg_catalog.coalesce(p_snapshot->'f5_apply','{}'::jsonb),'REV-F5-CERTIFIED','1','docs/control/SENTINEL_DATA_INTEGRITY_SIGNALS_CONTRACT.md#sen-dq-f5-004-apply_without_governance',pg_catalog.jsonb_build_array('ROLLED_BACK_CANARY_EVENTS_ARE_HISTORICAL_AND_EXCLUDED_FROM_ACTIVE_VIOLATION_COUNT')); v_signals:=v_signals||pg_catalog.jsonb_build_array(v_signal);
  if not (p_snapshot ? 'duplicate_profile') or pg_catalog.coalesce((p_snapshot#>>'{duplicate_profile,telemetry_available}')::boolean,false)=false then v_state:='UNKNOWN'; elsif pg_catalog.coalesce((p_snapshot#>>'{duplicate_profile,critical_drift_count}')::integer,0)>0 then v_state:='BROKEN'; elsif pg_catalog.coalesce((p_snapshot#>>'{duplicate_profile,abnormal_drift_count}')::integer,0)>0 then v_state:='DEGRADED'; else v_state:='OK'; end if;
  v_signal:=public.aos_sentinel_rev_f6_6_signal_envelope_v1('SEN-DQ-F5-005','DUPLICATE_PROFILE_DRIFT','REV-F5','REV-F5_DUPLICATE_PROFILE_CLASSIFICATION',v_state,'MEDIUM',pg_catalog.jsonb_build_object('classes',pg_catalog.jsonb_build_array('AUTO_ELIGIBLE_EXACT','REVIEW_STRONG','BLOCK_CONFLICT','NO_MERGE')),pg_catalog.coalesce(p_snapshot->'duplicate_profile','{}'::jsonb),'REV-F5-CERTIFIED','1','docs/control/SENTINEL_DATA_INTEGRITY_SIGNALS_CONTRACT.md#sen-dq-f5-005-duplicate_profile_drift',case when v_state='UNKNOWN' then pg_catalog.jsonb_build_array('REQUESTED_DUPLICATE_PROFILE_CLASSES_NOT_MATERIALIZED_IN_LIVE_SCHEMA','MISSING_TELEMETRY_NEVER_RETURNS_OK') else '[]'::jsonb end); v_signals:=v_signals||pg_catalog.jsonb_build_array(v_signal);
  if not (p_snapshot ? 'product_sale') then v_state:='UNKNOWN'; elsif pg_catalog.coalesce((p_snapshot#>>'{product_sale,missing_sale}')::bigint,0)>0 or pg_catalog.coalesce((p_snapshot#>>'{product_sale,resolved_missing_product}')::bigint,0)>0 then v_state:='BROKEN'; else v_state:='OK'; end if;
  v_signal:=public.aos_sentinel_rev_f6_6_signal_envelope_v1('SEN-DQ-REV-001','PRODUCT_SALE_ORPHAN','REV-F3','REV-F3_PRODUCT_SALE_FACT',v_state,'HIGH',pg_catalog.jsonb_build_object('missing_sale',0,'resolved_missing_product',0),pg_catalog.coalesce(p_snapshot->'product_sale','{}'::jsonb),'REV-F3-CERTIFIED','1','docs/control/SENTINEL_DATA_INTEGRITY_SIGNALS_CONTRACT.md#sen-dq-rev-001-product_sale_orphan'); v_signals:=v_signals||pg_catalog.jsonb_build_array(v_signal);
  if not (p_snapshot ? 'reconciliation') then v_state:='UNKNOWN'; elsif pg_catalog.coalesce((p_snapshot#>>'{reconciliation,missing_sale}')::bigint,0)>0 or pg_catalog.coalesce((p_snapshot#>>'{reconciliation,missing_payment}')::bigint,0)>0 or pg_catalog.coalesce((p_snapshot#>>'{reconciliation,missing_quote}')::bigint,0)>0 or pg_catalog.coalesce((p_snapshot#>>'{reconciliation,no_evidence_ref}')::bigint,0)>0 then v_state:='BROKEN'; else v_state:='OK'; end if;
  v_signal:=public.aos_sentinel_rev_f6_6_signal_envelope_v1('SEN-DQ-REV-002','RECONCILIATION_ORPHAN','REV-F4','REV-F4_RECONCILIATION',v_state,'HIGH',pg_catalog.jsonb_build_object('missing_sale',0,'missing_payment',0,'missing_quote',0,'no_evidence_ref',0),pg_catalog.coalesce(p_snapshot->'reconciliation','{}'::jsonb),'REV-F4-CERTIFIED','1','docs/control/SENTINEL_DATA_INTEGRITY_SIGNALS_CONTRACT.md#sen-dq-rev-002-reconciliation_orphan'); v_signals:=v_signals||pg_catalog.jsonb_build_array(v_signal);
  if not (p_snapshot ? 'f6_readmodel') or pg_catalog.coalesce((p_snapshot#>>'{f6_readmodel,cache_rows}')::integer,0)=0 then v_state:='UNKNOWN'; elsif pg_catalog.coalesce((p_snapshot#>>'{f6_readmodel,version_mismatch_count}')::integer,0)>0 then v_state:='BROKEN'; elsif pg_catalog.coalesce((p_snapshot#>>'{f6_readmodel,source_newer_than_cache}')::boolean,false) then v_state:='DEGRADED'; else v_state:='OK'; end if;
  v_signal:=public.aos_sentinel_rev_f6_6_signal_envelope_v1('SEN-DQ-F6-001','READMODEL_STALE','REV-F6','REV-F6.4_SALES_INTELLIGENCE_3_V1',v_state,'HIGH',pg_catalog.jsonb_build_object('version_mismatch_count',0,'source_newer_than_cache',false),pg_catalog.coalesce(p_snapshot->'f6_readmodel','{}'::jsonb),'REV-F6.4','1','docs/control/SENTINEL_DATA_INTEGRITY_SIGNALS_CONTRACT.md#sen-dq-f6-001-readmodel_stale',pg_catalog.jsonb_build_array('FRESHNESS_IS_SOURCE_AWARE_CACHE_GENERATED_AT_MUST_NOT_PRECEDE_RELEVANT_SOURCE_FRESHNESS')); v_signals:=v_signals||pg_catalog.jsonb_build_array(v_signal);
  if not (p_snapshot ? 'f6_coverage') then v_state:='UNKNOWN'; else v_missing:=pg_catalog.coalesce((p_snapshot#>>'{f6_coverage,missing_metrics}')::integer,0); v_max_drop:=pg_catalog.coalesce((p_snapshot#>>'{f6_coverage,max_drop_pp}')::numeric,0); if v_missing>0 then v_state:='UNKNOWN'; elsif v_max_drop>=pg_catalog.coalesce((v_b#>>'{coverage_material_drop_pp}')::numeric,2.0) then v_state:='BROKEN'; elsif v_max_drop>0 then v_state:='DEGRADED'; else v_state:='OK'; end if; end if;
  v_signal:=public.aos_sentinel_rev_f6_6_signal_envelope_v1('SEN-DQ-F6-002','COVERAGE_REGRESSION','REV-F6','REV-F6.3_METRIC_TRUST',v_state,'HIGH',pg_catalog.jsonb_build_object('baseline_pct',v_b->'coverage_pct','material_drop_pp',v_b->'coverage_material_drop_pp'),pg_catalog.coalesce(p_snapshot->'f6_coverage','{}'::jsonb),'REV-F6.3','1','docs/control/SENTINEL_DATA_INTEGRITY_SIGNALS_CONTRACT.md#sen-dq-f6-002-coverage_regression'); v_signals:=v_signals||pg_catalog.jsonb_build_array(v_signal);
  if not (p_snapshot ? 'patient360') or pg_catalog.coalesce((p_snapshot#>>'{patient360,metrics_missing}')::integer,0)>0 then v_state:='UNKNOWN'; elsif pg_catalog.coalesce((p_snapshot#>>'{patient360,resolved_collisions}')::integer,0)>0 or pg_catalog.coalesce((p_snapshot#>>'{patient360,resolved_missing_target}')::integer,0)>0 or pg_catalog.coalesce((p_snapshot#>>'{patient360,resolved_bad_candidate_count}')::integer,0)>0 then v_state:='BROKEN'; else v_max_conflict_delta:=pg_catalog.coalesce((p_snapshot#>>'{patient360,max_conflict_rate_delta_pp}')::numeric,0); if v_max_conflict_delta>=pg_catalog.coalesce((v_b#>>'{patient360_conflict_rate_material_delta_pp}')::numeric,1.0) then v_state:='DEGRADED'; else v_state:='OK'; end if; end if;
  v_signal:=public.aos_sentinel_rev_f6_6_signal_envelope_v1('SEN-DQ-360-001','PATIENT360_IDENTITY_RESOLUTION_REGRESSION','REV-F6.1','REV_PATIENT_COMMERCIAL_360_V2',v_state,'HIGH',pg_catalog.jsonb_build_object('alias_key_baseline',v_b->'patient360_alias_keys','material_conflict_rate_delta_pp',v_b->'patient360_conflict_rate_material_delta_pp','resolved_collisions',0,'resolved_missing_target',0,'resolved_bad_candidate_count',0),pg_catalog.coalesce(p_snapshot->'patient360','{}'::jsonb),'REV-F6.3','1','docs/control/SENTINEL_DATA_INTEGRITY_SIGNALS_CONTRACT.md#sen-dq-360-001-patient360_identity_resolution_regression',pg_catalog.jsonb_build_array('KNOWN_FAIL_CLOSED_CONFLICT_KEYS_ARE_NOT_RESOLUTION_ERRORS')); v_signals:=v_signals||pg_catalog.jsonb_build_array(v_signal);
...
begin
  with per_batch as (select b.id,b.source_rows,b.status,pg_catalog.coalesce((b.metadata->>'staging_complete')::boolean,false) staging_complete,count(s.*)::bigint persisted,count(distinct s.source_row_num)::bigint distinct_n,min(s.source_row_num) min_n,max(s.source_row_num) max_n from public.aos_f5_source_batches_v1 b left join public.aos_f5_patient_source_rows_v1 s on s.batch_id=b.id group by b.id,b.source_rows,b.status,b.metadata) select pg_catalog.jsonb_build_object('batches_total',count(*)::bigint,'staging_complete_batches',count(*) filter(where staging_complete)::bigint,'matched_batches',count(*) filter(where status='MATCHED')::bigint,'expected_rows',pg_catalog.coalesce(sum(source_rows),0)::bigint,'persisted_rows',pg_catalog.coalesce(sum(persisted),0)::bigint,'mismatched_batches',count(*) filter(where persisted<>source_rows)::bigint,'continuity_failures',count(*) filter(where persisted<>source_rows or distinct_n<>persisted or (persisted>0 and (max_n-min_n+1)<>persisted))::bigint) into v_source from per_batch;
