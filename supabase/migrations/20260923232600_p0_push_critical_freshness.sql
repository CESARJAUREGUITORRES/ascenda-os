-- P0 communications recovery · critical freshness ordering
-- During backlog recovery, current appointment lifecycle pushes must not wait
-- behind older team/admin digest traffic. Keep the five-row throttle from the
-- previous P0 and change ordering only; recipient/dedupe/delivery semantics stay intact.
...
begin
  with pick as (
    select n.id
     ...
     order by
       case when n.event_type in ('APPOINTMENT_CREATED','APPOINTMENT_RESCHEDULED','APPOINTMENT_CANCELLED','APPOINTMENT_NO_SHOW') then 0 else 1 end,
       case n.prioridad when 'URGENTE' then 0 when 'ALTA' then 1 else 2 end,
       case when n.event_type in ('APPOINTMENT_CREATED','APPOINTMENT_RESCHEDULED','APPOINTMENT_CANCELLED','APPOINTMENT_NO_SHOW') then n.created_at end desc nulls last,
       n.created_at asc
...
'Generic notification push claim. P0 2026-09-23 caps each claim at 5 and prioritizes fresh appointment lifecycle events ahead of stale digest backlog.';
