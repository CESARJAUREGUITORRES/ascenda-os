...
returns table(
  ...
  lifecycle text,
  engagement text,
  followup_pending integer,
  followup_overdue integer,
...
with owned as (
  ...
  select
    ...
    coalesce(ff.pending_count,0)::integer followup_pending,
    coalesce(ff.overdue_count,0)::integer followup_overdue,
    ff.next_followup_at,cf.latest_status latest_call_status,cf.last_call_at,
    coalesce(w.pinned,false) pinned,w.snoozed_until,
    coalesce(w.priority_override,'NORMAL') priority_override,
  ...
  left join public.aos_cia_call_facts_v1 cf on cf.contact_key=x.contact_key
  left join lateral (
    select
      max(e.occurred_at) filter(where e.event_type='CLAIM') last_claim_at,
    ...
    where e.assignment_id=x.id
  ) r on true
)
select
  e.assignment_id,e.plan_id,e.activation_id,e.contact_key,e.advisor_user_id,e.advisor_name,e.advisor_code,
  e.assignment_state,e.assigned_at,e.must_start_before,e.expires_at,e.started_at,e.completed_at,e.terminal_reason,
  e.contact_name,e.patient_state,e.branch,e.value_tier,e.lifecycle,e.engagement,e.followup_pending,e.followup_overdue,
  e.next_followup_at,e.latest_call_status,e.last_call_at,e.pinned,e.snoozed_until,e.priority_override,e.is_snoozed,
  case
  ...
  (
    ...
    + case e.value_tier when 'DIAMANTE' then 400 when 'GOLD' then 300 when 'PREMIUM' then 200 else 0 end
