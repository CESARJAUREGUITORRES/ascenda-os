-- K1-E — preserve the latest branded Auth V3 template after moving credentials
-- to the K1 private store. Functional session/challenge semantics remain Phase 2.
...
begin
  ...
  if v_attempts>=5 then return pg_catalog.jsonb_build_object('ok',false,'error','Cuenta bloqueada 15 min.'); end if;

  if not public.aos_auth_password_matches(v_user.codigo_asesor,p_password) then
    insert into public.aos_security_log(usuario,accion,detalles)
    values (v_user.nombre,'login_failed',pg_catalog.jsonb_build_object('reason','password','version','v3-k1'));
  ...
  limit 1;
  if v_udata.id is null then return pg_catalog.jsonb_build_object('ok',false,'error','Usuario activo no encontrado'); end if;
  ...
  if coalesce(v_udata.two_factor,false) then
    ...
    if v_api_key is null then
      ...
      insert into public.aos_security_log(usuario,accion,detalles)
      values(v_user.nombre,'2fa_delivery_failed',pg_catalog.jsonb_build_object('reason','provider_unavailable','version','v3-k1'));
    ...
    select net.http_post(
      ...
      headers:=pg_catalog.jsonb_build_object('Authorization','Bearer '||v_api_key,'Content-Type','application/json'),
    ...
    insert into public.aos_security_log(usuario,accion,detalles)
    values(v_user.nombre,'2fa_challenge_sent',pg_catalog.jsonb_build_object('challenge_id',v_challenge,'request_id',v_req,'version','v3-k1','template','ascenda_branded_v1'));
  ...
  insert into public.aos_security_log(usuario,accion,detalles)
  values(v_user.nombre,'login',pg_catalog.jsonb_build_object('method','password_v3_k1','expires_at',v_expires));
  return pg_catalog.jsonb_build_object(
