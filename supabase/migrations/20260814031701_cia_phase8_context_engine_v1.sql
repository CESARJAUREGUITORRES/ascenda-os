-- ASCENDA CIA Phase 8 — deterministic context/availability engine.
...
begin
  ...
  keys:=public.aos_cia_audience_resolve_node_v2(v_filter);
...
begin
  ...
  if ctx.policy_key in ('CALL_GENERAL','CALL_PROVINCE') then
    ...
    base as (
      select m.contact_key,
             ...
             coalesce(c.called_today,false) called_today,
             coalesce(a.has_future_appointment,false) future_appointment,
             s.lifecycle,
      ...
      left join (
        select distinct numero_limpio from public.aos_leads_en_curso
        where fecha=(now() at time zone 'America/Lima')::date
      ) l on l.numero_limpio=m.contact_key
    ), d as (
      select b.*,
        case
    ...
    select d.contact_key,d.elig,
      case when d.elig='UNKNOWN' then 'UNKNOWN'
  ...
  if ctx.policy_key='EMAIL_GENERAL' then
    ...
    base as (
      ...
      select b.*,
        case
          when b.canonical_email is null or position('@' in b.canonical_email)=0 or position(' ' in b.canonical_email)>0 then 'INELIGIBLE'
          when b.identity_confidence is null then 'UNKNOWN'
          when b.identity_confidence<>'MEDIUM' then 'INELIGIBLE'
          else 'ELIGIBLE' end elig,
        array_remove(array[
          case when b.canonical_email is null or position('@' in b.canonical_email)=0 or position(' ' in b.canonical_email)>0 then 'EMAIL_INVALID' end,
          ...
          case when b.days_since_last=0 then 'EMAIL_SENT_TODAY' end
        ],null) rs,
    ...
    select d.contact_key,d.elig,
      case when d.elig='UNKNOWN' then 'UNKNOWN'
           when d.elig='INELIGIBLE' then 'UNAVAILABLE'
           when d.days_since_last=0 then 'UNAVAILABLE'
           else 'AVAILABLE' end,
  ...
  if ctx.policy_key in ('SMS_GENERAL','WHATSAPP_GENERAL') then
    ...
    select m.contact_key,
      ...
      false,
      array_remove(array[
        case when m.contact_key !~ '^[0-9]{9}$' then 'PHONE_INVALID' end,
        case when m.contact_key ~ '^[0-9]{9}$' then 'CHANNEL_HISTORY_NOT_INTEGRATED' end
      ],null),array[]::text[],ctx.policy_key,ctx.policy_version,ctx.channel
    from public.aos_cia_activation_member_keys_v1(p_activation_id) m;
    return;
  end if;

  return query
  select m.contact_key,'ELIGIBLE'::text,'AVAILABLE'::text,
         (ctx.activation_state='ACTIVE'),array[]::text[],array[]::text[],ctx.policy_key,ctx.policy_version,ctx.channel
  from public.aos_cia_activation_member_keys_v1(p_activation_id) m;
end;
$$;
