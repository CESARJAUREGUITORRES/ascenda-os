-- WA-4C governed WhatsApp booking write.
-- HUMAN_ONLY boundary: this RPC commits a booking only after an explicit human action.
-- It does not send WhatsApp messages and does not create Call Center call records.
begin;

create extension if not exists pgcrypto;

create table if not exists public.aos_wa4_booking_actions_v1 (
  ...
  ad_id text,
  lead_id text,
  status text not null default 'BOOKED' check (status in ('BOOKED','CANCELLED','REPLACED')),
  created_at timestamptz not null default now()
);

create index if not exists idx_aos_wa4_booking_actions_conversation_v1
  on public.aos_wa4_booking_actions_v1(conversation_id,created_at desc);
create index if not exists idx_aos_wa4_booking_actions_agenda_v1
  on public.aos_wa4_booking_actions_v1(agenda_id);

alter table public.aos_wa4_booking_actions_v1 enable row level security;
revoke all on public.aos_wa4_booking_actions_v1 from public,anon,authenticated;
grant select,insert,update on public.aos_wa4_booking_actions_v1 to service_role;

create or replace function public.aos_wa4_require_governed_booking_v1()
returns trigger
language plpgsql
security definer
...
as $$
begin
  if upper(coalesce(new.origen_cita,''))='WHATSAPP'
     and coalesce(current_setting('aos.wa4_governed_booking_write',true),'')<>'1' then
    raise exception 'WA4_GOVERNED_BOOKING_WRITE_REQUIRED';
  end if;
  return new;
end
$$;

revoke all on function public.aos_wa4_require_governed_booking_v1() from public,anon,authenticated;

drop trigger if exists trg_001_aos_wa4_governed_booking_v1 on public.aos_agenda_citas;
create trigger trg_001_aos_wa4_governed_booking_v1
before insert or update on public.aos_agenda_citas
for each row execute function public.aos_wa4_require_governed_booking_v1();
...
begin
  ...
  if found then
    ...
    return jsonb_build_object(
      'ok',true,'status',v_existing.status,'agenda_id',v_existing.agenda_id,
      'idempotent_replay',true,'confirmation_allowed',true
    );
  end if;
  ...
  if not exists (
    ...
    where a.conversation_id=p_conversation_id
      and a.owner_user_id=p_actor_id
      and a.state='ACTIVE'
  ) then
  ...
  where id=nullif(btrim(p_payload->>'professional_id'),'')
  ...
  if upper(coalesce(v_prof.tipo,''))<>v_role then
    return jsonb_build_object('ok',false,'error','WA4_BOOKING_PROFESSIONAL_ROLE_MISMATCH');
  end if;
  if upper(coalesce(v_prof.sede,'')) not in (v_site,'TODAS') then
    return jsonb_build_object('ok',false,'error','WA4_BOOKING_PROFESSIONAL_SITE_MISMATCH');
  end if;

  select max(fecha) into v_latest_schedule from public.aos_horarios_personal where activo=true;
  if v_latest_schedule is null or v_latest_schedule < v_date then
    return jsonb_build_object('ok',false,'error','WA4_BOOKING_SCHEDULE_SOURCE_STALE','latest_schedule_date',v_latest_schedule,'requires_human',true);
  end if;
  ...
  if coalesce((v_slots->>'ok')::boolean,false) is not true then
    return jsonb_build_object('ok',false,'error','WA4_BOOKING_SLOT_AUTHORITY_UNAVAILABLE','requires_human',true);
  end if;
  select exists(
    select 1 from jsonb_array_elements(coalesce(v_slots->'slots','[]'::jsonb)) s
    where s->>'hora'=v_time_text and coalesce((s->>'disponible')::boolean,false)=true
  ) into v_slot_ok;
  ...
  v_prof_name := btrim(v_prof.nombre_publico);
  ...
  insert into public.aos_agenda_citas(
    ...
    case when v_role='ENFERMERIA' then v_prof_name else 'WHATSAPP' end,p_actor_id::text,'PENDIENTE',null,now(),now(),v_time_text,v_conv.campaign_source,
  ...
  insert into public.aos_wa4_booking_actions_v1(
    ...
    p_idempotency_key,v_hash,p_conversation_id,p_actor_id,v_agenda_id,v_treatment.id,v_prof.id,
  ...
  return jsonb_build_object(
    ...
    'date',v_date,'time',v_time_text,'professional_role',v_role,'source',v_source
  );
end
$$;
...
comment on table public.aos_wa4_booking_actions_v1 is
'WA-4C append-style booking action and WhatsApp/Meta attribution ledger. Contains no duplicated patient PII.';
comment on function public.aos_wa4_commit_booking_v1(uuid,text,uuid,jsonb) is
'WA-4C HUMAN_ONLY transactional booking commit: ownership, identity, role, fresh schedule, slot revalidation and idempotency. Does not send WhatsApp or fabricate Call Center calls.';
