-- ASCENDA CIA V3 — Phase 11 V3 routing core.
-- Private core. Public dispatcher is created separately.

create or replace function public.aos_cia_call_routing_resolve_advisor_v1(
  p_asesor text,
  p_id_asesor text
)
returns uuid
...
as $function$
  ...
  where u.activo=true
    ...
    and upper(trim(coalesce(u.codigo_asesor,'')))=upper(trim(coalesce(p_id_asesor,'')))
    and upper(trim(coalesce(u.nombre,'')))=upper(trim(coalesce(p_asesor,'')))
  limit 1;
$function$;
...
declare
  advisor_id uuid;
  readiness jsonb;
  lima_today date := (clock_timestamp() at time zone 'America/Lima')::date;
...
begin
  ...
  -- V3 clinic-day is always America/Lima. p_hoy is retained only for compatibility metadata.
  effective_day := lima_today;
  ...
  for cand in
    ...
    limit 8
  loop
    -- Availability remains authoritative in F8 even after assignment.
    if exists(
      ...
      -- Recheck legacy transient claim after acquiring per-contact lock.
      if exists(
        select 1 from public.aos_leads_en_curso lc
        where lc.fecha=effective_day
  ...
  if picked.state='ASSIGNED' then
    transition_result := public.aos_cia_assignment_lease_transition_internal_v1(
      picked.id,'START',advisor_id,'CALL_ROUTER_V3_CLAIM'
    );
  ...
  select ld.id,ld.numero_limpio,coalesce(ld.tratamiento,'') tratamiento,coalesce(ld.anuncio,'') anuncio,
         ld.fecha,ld.hora_ingreso
  into lead_row
  from public.aos_leads ld
  where ld.numero_limpio in (picked.contact_key,'51'||picked.contact_key)
  order by ld.fecha desc,ld.hora_ingreso desc nulls last,ld.id desc
  limit 1;

  select count(*)::integer+1 into attempts
  from public.aos_llamadas ll
  where ll.numero_limpio in (picked.contact_key,'51'||picked.contact_key);

  select ll.fecha,ll.estado,ll.asesor,ll.observacion,ll.intento
  into last_call
  from public.aos_llamadas ll
  where ll.numero_limpio in (picked.contact_key,'51'||picked.contact_key)
  ...
  response_json := jsonb_build_object(
    ...
    'lead',jsonb_build_object(
      'id',lead_row.id,
      'num',picked.contact_key,
      'trat',coalesce(lead_row.tratamiento,''),
      'anuncio',coalesce(lead_row.anuncio,''),
      'fecha',coalesce(lead_row.fecha,effective_day),
      'hora_ingreso',lead_row.hora_ingreso,
      'intento',coalesce(attempts,1),
      'rowNum',0,
      'attributionSource','CIA_ASSIGNMENT_V3',
    ...
    'routingV3',jsonb_build_object(
      'route','V3','mode',upper(coalesce(p_mode,'V3_CANARY')),'fallbackUsed',false,
      'assignmentId',picked.id,'planId',picked.plan_id,'activationId',picked.activation_id,
      'advisorUserId',advisor_id,'contactKey',picked.contact_key,
      'state','IN_PROGRESS','sourceRank',picked.source_rank,
      'mustStartBefore',picked.must_start_before,'expiresAt',picked.expires_at,
      'clinicDay',effective_day,'clientDay',p_hoy,'latencyMs',elapsed
    )
  );

  insert into public.aos_cia_call_routing_events(
    event_type,advisor_user_id,routing_mode,route_selected,assignment_id,plan_id,activation_id,
    contact_key,latency_ms,payload
  ) values(
    'CLAIM',advisor_id,upper(coalesce(p_mode,'V3_CANARY')),'V3',picked.id,picked.plan_id,picked.activation_id,
    picked.contact_key,elapsed,jsonb_build_object('clinic_day',effective_day,'client_day',p_hoy,'previous_state',picked.state)
  );
...
revoke execute on function public.aos_cia_call_routing_resolve_advisor_v1(text,text) from public, anon, authenticated;
revoke execute on function public.aos_cia_call_routing_try_v3_v1(text,text,date,text) from public, anon, authenticated;
grant execute on function public.aos_cia_call_routing_resolve_advisor_v1(text,text) to service_role;
