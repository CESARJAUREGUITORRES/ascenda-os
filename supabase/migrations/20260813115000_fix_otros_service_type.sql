-- ASCENDA OS
-- Hotfix: OTROS must always be classified as SERVICIO.
-- Risk: HIGH/CRITICAL because it touches aos_ventas and replaces a SECURITY DEFINER RPC.
-- Validated first with BEGIN/ROLLBACK against production schema on 2026-08-13.

-- Guardrail: expected baseline is 11 bad rows. Abort if the scope grows unexpectedly.
DO $$
DECLARE
  v_bad integer;
BEGIN
...
-- Central invariant: regardless of caller/importer/frontend, OTROS cannot persist as PRODUCTO.
...
-- Fix admin sales editor contract: frontend already sends `tipo`, but the RPC allowlist omitted it.
...
-- Record exactly which rows are normalized, without duplicating PII into GitHub.
INSERT INTO public.aos_auditoria_ediciones
  (tabla, registro_id, campo, valor_anterior, valor_nuevo, editado_por, origen)
SELECT
  'aos_ventas',
...
-- Postcondition: database invariant must hold immediately after migration.
DO $$
DECLARE
  v_remaining integer;
BEGIN
