-- AGV2 legacy-treatment rebook bridge V2.
-- Date-specific staff schedules are authoritative. Generic site hours are fallback only.
...
begin
  ...
  if v_site=upper(replace(btrim(coalesce(v_cita.sede,'')),'_',' '))
    ...
    return jsonb_build_object('ok',true,'status','NO_CHANGE','appointment_id',p_appointment_id,'bridge_mode','LEGACY_SAFE_V2','google_queue_required',false);
  end if;
  ...
  if v_role='DOCTORA' then
    ...
    select exists(
      ...
      where h.fecha=v_date
        and upper(h.sede)=v_site
        and upper(regexp_replace(coalesce(h.rol,''),'[^A-Z]','','g'))='ENFERMERIA'
        and h.activo=true
    ) into v_has_role_schedule;
    ...
    if v_has_role_schedule then
      ...
      if not v_has_slot_schedule then
        return jsonb_build_object('ok',false,'error','AGV2_LEGACY_NO_STAFF_AT_TIME','site',v_site,'date',v_date,'time',to_char(v_time,'HH24:MI'));
      end if;
    else
      if v_cfg.id is null or v_cfg.activo is not true then
        return jsonb_build_object('ok',false,'error','AGV2_LEGACY_SITE_CLOSED','site',v_site,'date',v_date);
  ...
  return jsonb_build_object(
    ...
    'bridge_mode','LEGACY_SAFE_V2','google_queue_required',true,
