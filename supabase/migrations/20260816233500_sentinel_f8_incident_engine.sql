...
-- DESIGN/BRANCH ONLY until explicit production authorization.
-- Zero-PHI/PII: stores normalized technical metadata and typed evidence references only.
...
create index aos_sentinel_incidents_v1_updated_idx
  on public.aos_sentinel_incidents_v1(updated_at desc);
create index aos_sentinel_incidents_v1_status_severity_idx
  on public.aos_sentinel_incidents_v1(status, severity, updated_at desc);
...
create index aos_sentinel_incident_signals_v1_incident_idx
  on public.aos_sentinel_incident_signals_v1(incident_id, observed_at);
...
create index aos_sentinel_incident_timeline_v1_incident_idx
  on public.aos_sentinel_incident_timeline_v1(incident_id, timeline_id);
...
begin
  ...
  loop
    ...
    if (select pg_catalog.count(*) from pg_catalog.jsonb_object_keys(v)) <> 2 then return false; end if;
    v_kind := v->>'kind';
    v_id := v->>'id';
...
begin
  ...
  for k in select key from pg_catalog.jsonb_object_keys(p_corr) as key
  loop
...
begin
  ...
  for k in select key from pg_catalog.jsonb_object_keys(p_signal) as key
  loop
    if k not in ('event_id','signal_class','environment','domain','component','capability','failure_family','signal_fingerprint','incident_fingerprint','severity','observed_at','evidence_refs','correlation') then
      raise exception 'F8_SIGNAL_UNAPPROVED_KEY:%', k;
    end if;
  end loop;
  if not (p_signal ?& array['event_id','signal_class','environment','domain','component','capability','failure_family','signal_fingerprint','incident_fingerprint','severity','observed_at']) then
    raise exception 'F8_SIGNAL_REQUIRED_FIELD_MISSING';
  end if;

  v_event_id := p_signal->>'event_id';
  v_class := pg_catalog.upper(p_signal->>'signal_class');
  v_env := pg_catalog.lower(p_signal->>'environment');
  v_domain := pg_catalog.upper(p_signal->>'domain');
  v_component := pg_catalog.lower(p_signal->>'component');
  v_capability := pg_catalog.lower(p_signal->>'capability');
  v_failure := pg_catalog.lower(p_signal->>'failure_family');
  v_signal_fp := pg_catalog.lower(p_signal->>'signal_fingerprint');
  v_incident_fp := pg_catalog.lower(p_signal->>'incident_fingerprint');
  v_severity := pg_catalog.upper(p_signal->>'severity');
  begin
    v_observed := (p_signal->>'observed_at')::timestamptz;
  exception when others then
    raise exception 'F8_SIGNAL_INVALID_TIMESTAMP';
  end;
  v_evidence := pg_catalog.coalesce(p_signal->'evidence_refs','[]'::jsonb);
  v_corr := p_signal->'correlation';

  if v_event_id !~ '^[A-Za-z0-9._:@/-]{1,200}$' or v_event_id ~ '[?#]' or v_event_id ~ '\.\.' then raise exception 'F8_EVENT_ID_INVALID'; end if;
  if v_class not in ('ERROR','AVAILABILITY','BUSINESS_HEALTH','DEPENDENCY','DEPLOYMENT_CHANGE','SECURITY','USER_REPORTED') then raise exception 'F8_SIGNAL_CLASS_INVALID'; end if;
  if v_env not in ('production','zero-cost','development') then raise exception 'F8_ENVIRONMENT_INVALID'; end if;
  if v_domain !~ '^[A-Z][A-Z0-9_]{0,63}$' then raise exception 'F8_DOMAIN_INVALID'; end if;
  if v_component !~ '^[a-z0-9][a-z0-9._:-]{0,199}$' or v_capability !~ '^[a-z0-9][a-z0-9._:-]{0,199}$' or v_failure !~ '^[a-z0-9][a-z0-9._:-]{0,199}$' then raise exception 'F8_TAXONOMY_INVALID'; end if;
  if v_signal_fp !~ '^[a-z0-9][a-z0-9._:-]{0,199}$' or v_incident_fp !~ '^[a-z0-9][a-z0-9._:-]{0,199}$' then raise exception 'F8_FINGERPRINT_INVALID'; end if;
  if v_severity not in ('P0','P1','P2','P3') then raise exception 'F8_SEVERITY_INVALID'; end if;
  ...
  select s.incident_id into v_existing_incident_id from public.aos_sentinel_incident_signals_v1 s where s.event_id=v_event_id;
  if v_existing_incident_id is not null then
    select pg_catalog.to_jsonb(i) into v_corr from public.aos_sentinel_incidents_v1 i where i.incident_id=v_existing_incident_id;
    return pg_catalog.jsonb_build_object('ok',true,'replay',true,'mutated',false,'reopened',false,'incident',v_corr);
  end if;
  ...
  if v_incident_id is null then
    select i.incident_id,i.resolved_at into v_resolved_id,v_resolved_at
    from public.aos_sentinel_incidents_v1 i
    where i.environment=v_env and i.incident_fingerprint=v_incident_fp and i.status='RESOLVED'
    ...
    if v_resolved_id is not null and v_resolved_at is not null and v_observed >= v_resolved_at and v_observed <= v_resolved_at + interval '60 minutes' then
      v_incident_id := v_resolved_id;
      v_reopened := true;
      update public.aos_sentinel_incidents_v1
      set status='OPEN',resolved_at=null,reopened_count=reopened_count+1,updated_at=v_now
  ...
  if v_incident_id is null then
    v_year := pg_catalog.extract(year from v_observed)::integer;
    ...
    insert into public.aos_sentinel_incidents_v1(
      incident_id,incident_fingerprint,environment,domain,component,capability,failure_family,severity,status,
      opened_at,updated_at,last_signal_at,resolved_at,signal_count,reopened_count,signal_classes,signal_fingerprints,evidence_refs,correlation
    ) values(
      v_incident_id,v_incident_fp,v_env,v_domain,v_component,v_capability,v_failure,v_severity,'OPEN',
      v_observed,v_now,v_observed,null,1,0,array[v_class],array[v_signal_fp],v_evidence,v_corr
    );
    insert into public.aos_sentinel_incident_timeline_v1(incident_id,event_type,occurred_at,details)
    ...
    if v_incident.environment<>v_env or v_incident.domain<>v_domain or v_incident.component<>v_component or v_incident.capability<>v_capability or v_incident.failure_family<>v_failure or v_incident.incident_fingerprint<>v_incident_fp then
      raise exception 'F8_INCIDENT_FINGERPRINT_SCOPE_CONTRADICTION';
    end if;
    ...
    select pg_catalog.coalesce(pg_catalog.jsonb_agg(x.value order by x.value::text),'[]'::jsonb) into v_merged_evidence
    ...
    set severity=v_next_severity,
        updated_at=v_now,
        last_signal_at=pg_catalog.greatest(last_signal_at,v_observed),
        signal_count=signal_count+1,
        signal_classes=(select pg_catalog.array_agg(x order by x) from (select distinct pg_catalog.unnest(signal_classes || array[v_class]) as x) q),
        signal_fingerprints=(select pg_catalog.array_agg(x order by x) from (select distinct pg_catalog.unnest(signal_fingerprints || array[v_signal_fp]) as x) q),
        evidence_refs=v_merged_evidence,
        correlation=case when v_corr is null then correlation else v_corr end
    where incident_id=v_incident_id;
  ...
  select pg_catalog.to_jsonb(i) into v_corr from public.aos_sentinel_incidents_v1 i where i.incident_id=v_incident_id;
  return pg_catalog.jsonb_build_object('ok',true,'replay',false,'mutated',true,'reopened',v_reopened,'created',v_new,'incident',v_corr);
end;
$$;
...
declare
  ...
  v_target text := pg_catalog.upper(pg_catalog.coalesce(p_target_status,''));
  v_at timestamptz := pg_catalog.coalesce(p_at,pg_catalog.clock_timestamp());
...
begin
  ...
  v_allowed :=
    (v.status='OPEN' and v_target in ('ACK','INVESTIGATING','MITIGATED','RESOLVED')) or
    (v.status='ACK' and v_target in ('INVESTIGATING','MITIGATED','RESOLVED')) or
    (v.status='INVESTIGATING' and v_target in ('MITIGATED','RESOLVED')) or
    (v.status='MITIGATED' and v_target in ('INVESTIGATING','RESOLVED'));
  if not v_allowed then raise exception 'F8_STATUS_TRANSITION_INVALID:%->%',v.status,v_target; end if;
  update public.aos_sentinel_incidents_v1
  set status=v_target,updated_at=v_at,resolved_at=case when v_target='RESOLVED' then v_at else null end
  where incident_id=p_incident_id
  returning * into v;
  insert into public.aos_sentinel_incident_timeline_v1(incident_id,event_type,occurred_at,details)
  values(p_incident_id,'STATUS_CHANGED',v_at,pg_catalog.jsonb_build_object('from',case when v_target=v.status then v_target else null end));
  -- Replace the preceding compact placeholder with the real previous state from timeline-safe data.
  update public.aos_sentinel_incident_timeline_v1
  set details=pg_catalog.jsonb_build_object('from',
      case
        when v_target='ACK' then 'OPEN'
        when v_target='INVESTIGATING' then case when exists(select 1 from public.aos_sentinel_incident_timeline_v1 t where t.incident_id=p_incident_id and t.event_type='STATUS_CHANGED' and t.timeline_id < public.aos_sentinel_incident_timeline_v1.timeline_id and t.details->>'to'='MITIGATED') then 'MITIGATED' else 'ACK' end
        when v_target='MITIGATED' then 'INVESTIGATING'
        when v_target='RESOLVED' then 'ACTIVE'
        else 'UNKNOWN'
      end,
      'to',v_target)
  where timeline_id=(select max(timeline_id) from public.aos_sentinel_incident_timeline_v1 where incident_id=p_incident_id);
...
as $$
  select pg_catalog.jsonb_build_object(
    ...
    'signals',pg_catalog.coalesce((select pg_catalog.jsonb_agg(pg_catalog.to_jsonb(s) order by s.recorded_at,s.event_id) from public.aos_sentinel_incident_signals_v1 s where s.incident_id=i.incident_id),'[]'::jsonb),
    'timeline',pg_catalog.coalesce((select pg_catalog.jsonb_agg(pg_catalog.to_jsonb(t) order by t.timeline_id) from public.aos_sentinel_incident_timeline_v1 t where t.incident_id=i.incident_id),'[]'::jsonb)
  )
...
comment on function public.aos_sentinel_ingest_signal_v1(jsonb) is 'Sentinel F8 transactional ingest. service_role only. Idempotent by event_id; serializes incident fingerprint races.';
