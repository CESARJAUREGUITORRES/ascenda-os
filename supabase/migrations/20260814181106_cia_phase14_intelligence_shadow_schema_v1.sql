...
create table if not exists public.aos_cia_intelligence_recommendations (
  ...
  opportunity_type text not null check (opportunity_type in ('UNWORKED_LEAD','FOLLOWUP_RECOVERY','REACTIVATION','REPURCHASE_SIGNAL','HIGH_VALUE_ATTENTION')),
...
create index if not exists idx_cia_intelligence_runs_status_completed on public.aos_cia_intelligence_shadow_runs(status,completed_at desc);
create index if not exists idx_cia_intelligence_recs_run_priority on public.aos_cia_intelligence_recommendations(run_id,priority_score desc,created_at desc);
create index if not exists idx_cia_intelligence_recs_advisor on public.aos_cia_intelligence_recommendations(advisor_user_id,run_id,priority_score desc) where advisor_user_id is not null;
create index if not exists idx_cia_intelligence_recs_assignment on public.aos_cia_intelligence_recommendations(assignment_id) where assignment_id is not null;
create index if not exists idx_cia_intelligence_events_rec on public.aos_cia_intelligence_events(recommendation_id,occurred_at,id);
create index if not exists idx_cia_intelligence_events_request on public.aos_cia_intelligence_events(request_id,occurred_at,id) where request_id is not null;
