create table if not exists public.aos_audiencia_snapshots (
  ...
  constraint aos_audiencia_snapshots_membership_hash_chk check ((estado='BUILDING' and membership_hash='') or (estado='READY' and membership_hash ~ '^[0-9a-f]{64}$')),
  constraint aos_audiencia_snapshots_filter_hash_chk check (filter_hash ~ '^[0-9a-f]{64}$'),
  constraint aos_audiencia_snapshots_seal_chk check ((estado='BUILDING' and sealed_at is null) or (estado='READY' and sealed_at is not null))
);