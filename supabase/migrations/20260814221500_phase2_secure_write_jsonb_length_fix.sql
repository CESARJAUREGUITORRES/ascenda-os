-- ASCENDA OS — Phase 2 prerequisite repair discovered by F4 regression.
-- PostgreSQL has no jsonb_object_length(jsonb). Preserve the existing secure-write
-- authorization/table allowlist while validating PATCH/DELETE match cardinality
-- through jsonb_object_keys(). Additive CREATE OR REPLACE only; no business rows touched.
