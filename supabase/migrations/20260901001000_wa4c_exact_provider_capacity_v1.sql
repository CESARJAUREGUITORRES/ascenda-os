-- WA-4C exact-provider capacity hardening.
-- Nursing is explicitly allowed to attend two concurrent patients per eligible nurse.
-- Doctors are exact providers and must not be overbooked; capacity is one per 30-minute slot.
begin;
        ...
        while v_time + v_step <= v_h.hora_fin::time loop
          ...
          if v_occupied < 1 then
            v_slots:=v_slots||jsonb_build_array(jsonb_build_object(
              ...
              'libres',1-v_occupied,'capacidad',1,
...
begin
  ...
  if upper(coalesce(v_prof.tipo,''))='DOCTORA' then v_cap:=1; v_step:=interval '30 minutes';
...
'WA-4C booking authority: doctors exact-provider capacity 1; nursing site-pool capacity 2 per eligible nurse; Team skill + Catalog role + date/site schedule required.';
