...
begin
  ...
  if v_user.codigo_asesor is null then
    ...
    values (pg_catalog.left(pg_catalog.btrim(p_usuario),120),'login_failed',pg_catalog.jsonb_build_object('reason','not_found','version','v3'));
  ...
  if coalesce(v_udata.two_factor,false) then
    if coalesce(pg_catalog.btrim(v_udata.email),'')='' then
