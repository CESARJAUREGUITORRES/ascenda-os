...
begin
  ...
  if v_action='SUMMARY' then
    ...
    if v_action='APPROVE' and r.state='APPROVED' then
      return jsonb_build_object('ok',true,'request_id',r.id,'state','APPROVED','idempotent',true,'policy',v_policy);
    end if;
    if v_action='EXECUTE' and r.state='EXECUTED' then
      return jsonb_build_object('ok',true,'request_id',r.id,'state','EXECUTED','idempotent',true,'policy',v_policy);
    end if;
