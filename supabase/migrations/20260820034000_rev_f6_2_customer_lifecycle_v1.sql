-- REV-F6.2 — Customer Lifecycle V1
-- Derived/read-only analytics over certified F5/F6.1 identity + observed patient activity.
-- No patient merge, sale mutation, financial inference, or unsupported historical revenue claim.
...
comment on view public.aos_rev_customer_agenda_identity_v1 is
'REV-F6.2 private agenda→canonical identity bridge. Multiple strong identifiers that disagree fail closed.';
...
select distinct
  a.canonical_patient_id::text,
  a.fecha_cita::date,
  'ATTENDED_APPOINTMENT'::text,
  'AGENDA_IDENTITY_BRIDGE_V2'::text,
...
select distinct
  j.canonical_patient_id::text,
  v.fecha::date,
  'CANONICAL_SALE'::text,
  'F5_SALE_MATCH'::text,
  'TRANSACTION_2026'::text
...
comment on view public.aos_rev_customer_lifecycle_events_v1 is
'REV-F6.2 qualifying patient activity: F5 historical appointment evidence, safely-resolved attended/effective agenda events, and canonical matched sales. Historical appointment evidence never becomes revenue.';
...
create or replace function public.aos_rev_customer_lifecycle_by_patient_v1(p_canonical_patient_id text,p_as_of date default current_date)
returns jsonb
language plpgsql
stable
security definer
...
begin
  select exists(
    ...
    where p."ID_PACIENTE"=p_canonical_patient_id and coalesce(p."ESTADO_PACIENTE",'')<>'FUSIONADO'
  ) into v_exists;

  if not v_exists then
    return jsonb_build_object(
      'contract','REV-F6.2_CUSTOMER_LIFECYCLE_V1','as_of',v_as_of,
      'canonical_patient_id',p_canonical_patient_id,'lifecycle_state',null,
      'classification_status','CANONICAL_TARGET_MISSING','claim_strength','UNRESOLVED'
    );
  end if;
  ...
  if v_last_event is not null then
    select max(e.event_date) into v_previous_event
    from public.aos_rev_customer_lifecycle_events_v1 e
    where e.canonical_patient_id=p_canonical_patient_id and e.event_date<v_last_event and e.event_date<=v_as_of;
    v_recency := v_as_of-v_last_event;
    if v_previous_event is not null then v_gap := v_last_event-v_previous_event; end if;
  end if;

  select exists(
           select 1 from public.aos_rev_customer_agenda_identity_v1 a
           where a.identity_status='RESOLVED' and a.canonical_patient_id=p_canonical_patient_id
             and a.estado_cita='CITA CONFIRMADA' and a.fecha_cita>v_as_of
         ),
  ...
  into v_future,v_future_date
  from public.aos_rev_customer_agenda_identity_v1 a;

  if v_event_rows=0 then
    v_state := null;
    v_classification_status := 'INSUFFICIENT_ACTIVITY_EVIDENCE';
    v_claim_strength := 'UNRESOLVED';
  elsif v_previous_event is not null and v_gap>=180 and v_recency between 0 and 30 then
    v_state := 'HISTORICAL_REACTIVATED';
    v_classification_status := 'CLASSIFIED';
    v_claim_strength := case when v_hist_events>0 then 'HIGH' else 'MEDIUM' end;
  elsif v_active_days=1 and v_recency between 0 and 90 then
    v_state := 'NEW_PATIENT';
    v_classification_status := 'CLASSIFIED';
    v_claim_strength := case when v_hist_events>0 then 'MEDIUM' else 'LOW' end;
  elsif v_active_days>=2 and v_recency between 0 and 90 then
    v_state := 'ACTIVE_REPEAT';
    v_classification_status := 'CLASSIFIED';
    v_claim_strength := case when v_hist_events>0 then 'HIGH' else 'MEDIUM' end;
    ...
    v_state := 'RETURNING_PATIENT';
    v_classification_status := 'CLASSIFIED';
    v_claim_strength := case when v_hist_events>0 or v_active_days>=2 then 'MEDIUM' else 'LOW' end;
  end if;

  return jsonb_build_object(
    'contract','REV-F6.2_CUSTOMER_LIFECYCLE_V1',
    'as_of',v_as_of,
    ...
    'event_definition',jsonb_build_object(
      'qualifying',jsonb_build_array('F5_REVIEWED_MATCH:HISTORICAL_APPOINTMENT','AGENDA:ASISTIO','AGENDA:EFECTIVA','F5_MATCH:CANONICAL_SALE'),
      'future_confirmed_appointment_status','CITA CONFIRMADA',
      'registration_is_not_qualifying',true
    ),
    'evidence',jsonb_build_object(
      'event_rows',v_event_rows,'active_days',v_active_days,'first_observed_event',v_first_event,'last_observed_event',v_last_event,
      'previous_observed_event',v_previous_event,'historical_patient_events',v_hist_events,'agenda_attended_events',v_agenda_events,
      'observed_sales',v_sale_events,'observed_active_months',v_observed_months
    ),
    'flags',jsonb_build_object(
      'is_first_observed_sale',(v_sale_events=1),'is_repeat_sale',(v_sale_events>=2),'reactivation_gap_days',v_gap,
      'has_future_appointment',v_future,'next_confirmed_appointment',v_future_date,'observed_purchase_count',v_sale_events,
      'observed_active_months',v_observed_months,'recency_days',v_recency
    ),
    'coverage',jsonb_build_object(
      'patient_activity_scope','F5 reviewed patient-history appointment evidence + safely resolved Agenda + canonical matched sales',
      'patient_history_linkage','PARTIAL_SAFE_MATCH_ONLY',
      'transaction_scope','2026_ONLY',
      'transaction_min_date',(select min(v.fecha) from public.aos_ventas v),
      'transaction_max_date',(select max(v.fecha) from public.aos_ventas v),
...
$$;
comment on function public.aos_rev_customer_lifecycle_by_patient_v1(text,date) is
'REV-F6.2 deterministic canonical-patient lifecycle. Null state is fail-closed when no qualifying activity evidence exists.';
...
create or replace function public.aos_rev_customer_lifecycle_v1(p_lookup_type text,p_lookup_value text,p_as_of date default current_date)
returns jsonb
language plpgsql
stable
security definer
...
begin
  ...
  if coalesce(v_resolution->>'status','')<>'MATCH' then
    return jsonb_build_object(
      'contract','REV-F6.2_CUSTOMER_LIFECYCLE_V1','as_of',coalesce(p_as_of,current_date),
      'canonical_patient_id',null,'lifecycle_state','UNRESOLVED_IDENTITY','classification_status','IDENTITY_NOT_RESOLVED',
      'claim_strength','UNRESOLVED','identity_resolution_status',v_resolution->>'status','candidate_count',coalesce((v_resolution->>'candidate_count')::integer,0),
      'historical_warning','No patient-level repeat/lifetime claim is allowed without one safe canonical identity'
    );
  end if;
  v_result := public.aos_rev_customer_lifecycle_by_patient_v1(v_resolution->>'canonical_patient_id',coalesce(p_as_of,current_date));
  return v_result || jsonb_build_object('identity_resolution_status','MATCH');
end;
$$;
comment on function public.aos_rev_customer_lifecycle_v1(text,text,date) is
'REV-F6.2 service-only lookup lifecycle resolver. Identity conflicts/unresolved identifiers always become UNRESOLVED_IDENTITY.';
...
create or replace function public.aos_rev_customer_lifecycle_summary_v1(p_as_of date default current_date)
returns jsonb
language sql
stable
security definer
set search_path=''
as $$
with params as (select coalesce(p_as_of,current_date) as as_of),
patients as (
  select p."ID_PACIENTE"::text patient_id from public.aos_pacientes p where coalesce(p."ESTADO_PACIENTE",'')<>'FUSIONADO'
),
ev as (
  select e.* from public.aos_rev_customer_lifecycle_events_v1 e,params x where e.event_date<=x.as_of
),
dates as (
  select canonical_patient_id,array_agg(distinct event_date order by event_date desc) event_dates,
         count(*)::integer event_rows,
         count(*) filter(where event_type='HISTORICAL_APPOINTMENT')::integer hist_events,
         count(*) filter(where event_type='CANONICAL_SALE')::integer sale_events
  from ev group by canonical_patient_id
),future as (
  select a.canonical_patient_id,min(a.fecha_cita) next_confirmed
  from public.aos_rev_customer_agenda_identity_v1 a,params x
  where a.identity_status='RESOLVED' and a.estado_cita='CITA CONFIRMADA' and a.fecha_cita>x.as_of
  ...
  select lifecycle_state,count(*)::integer c from classified where lifecycle_state is not null group by lifecycle_state
)
select jsonb_build_object(
  'contract','REV-F6.2_CUSTOMER_LIFECYCLE_V1','as_of',(select as_of from params),
  'canonical_population',(select count(*) from patients),
  'classified_population',(select count(*) from classified where lifecycle_state is not null),
  'insufficient_activity_evidence',(select count(*) from classified where lifecycle_state is null),
  'states',coalesce((select jsonb_object_agg(lifecycle_state,c order by lifecycle_state) from state_counts),'{}'::jsonb),
  'thresholds',jsonb_build_object('active_days',90,'dormant_gap_days',180,'reactivation_gap_days',180,'reactivation_window_days',30),
  'qualifying_event_rows',(select count(*) from ev),
  'historical_warning','2024/2025 transactional sales = NO_CERTIFIED_SOURCE; state counts are observed lifecycle, not complete historical revenue cohorts'
);
$$;
comment on function public.aos_rev_customer_lifecycle_summary_v1(date) is
'REV-F6.2 aggregate zero-PII lifecycle summary for an explicit as-of date.';
...
-- Preserve F6.1 implementation as a private base and keep the existing browser RPC name as the only governed gateway.
do $$
begin
  if to_regprocedure('public.aos_patient_commercial_360_v2_f6_1_base(text,text,text)') is null then
    if to_regprocedure('public.aos_patient_commercial_360_v2(text,text,text)') is null then
      raise exception 'REV_F6_2_REQUIRES_F6_1_360';
    end if;
    alter function public.aos_patient_commercial_360_v2(text,text,text) rename to aos_patient_commercial_360_v2_f6_1_base;
  end if;
end $$;
revoke all on function public.aos_patient_commercial_360_v2_f6_1_base(text,text,text) from public,anon,authenticated;
grant execute on function public.aos_patient_commercial_360_v2_f6_1_base(text,text,text) to service_role;
...
begin
  ...
  if coalesce((v_base->>'found')::boolean,false) then
    ...
    v_lifecycle := public.aos_rev_customer_lifecycle_by_patient_v1(v_pid,current_date);
    ...
    v_lifecycle := public.aos_rev_customer_lifecycle_v1(p_lookup_type,p_lookup_value,current_date);
  end if;
...
'REV-F6.2 governed Patient Commercial 360 gateway. F6.1 canonical facts plus deterministic lifecycle; Auth V3 + PASSWORD_2FA remains enforced by the private F6.1 base.';
