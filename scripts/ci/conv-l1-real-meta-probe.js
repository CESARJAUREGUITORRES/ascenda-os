...
function assertEnv(){
  ...
  if(!['health','send'].includes(ACTION))throw new Error('INVALID_ACTION');
}
...
function requestJson(urlString,options){
  return new Promise((resolve,reject)=>{
    ...
    q.on('timeout',()=>q.destroy(new Error('HTTP_TIMEOUT')));
    q.on('error',reject);
...
async function provider(method,path,body){
  const r=await requestJson(BASE_URL+path,{method,body,headers:{'x-aos-wa-auto-token':INTERNAL_TOKEN},timeout:22000});
  return r;
}
function redactSummary(obj){
  ...
  return {
    ...
    credentialState:obj.credentialState||null,
    assetState:obj.assetState||null,
    permissionsState:obj.permissionsState||null,
    businessAccountAvailable:obj.businessAccountAvailable===true,
    gateway:obj.gateway||null,
    count:Number.isFinite(Number(obj.count))?Number(obj.count):undefined,
...
async function safety(){
  ...
  const out={
    mode:a&&a[0]&&a[0].mode,
    kill:!!(a&&a[0]&&a[0].kill_switch_engaged),
    autoReply:!!(ai&&ai[0]&&ai[0].auto_reply_enabled),
    aiSend:!!(routing&&routing[0]&&routing[0].ai_send_enabled),
    autoRouting:!!(routing&&routing[0]&&routing[0].auto_routing_enabled),
    humanSend:!!(routing&&routing[0]&&routing[0].human_send_enabled),
    activeAllowlist:Array.isArray(allow)?allow.length:-1
  };
...
async function target(){
  const rows=await sb('GET','/rest/v1/aos_wa_conversations_v1?id=eq.'+encodeURIComponent(CONVERSATION_ID)+'&select=id,contact_number,contact_name,state,phone_number_id&limit=1');
  ...
  const to=wa.phoneValue(row.contact_number);
  if(!to)throw new Error('TEST_CONVERSATION_PHONE_REQUIRED');
  return {id:row.id,to,state:row.state||null};
}
...
async function markRequest(key,state,messageId,error){
  await sb('PATCH','/rest/v1/aos_wa_outbound_requests_v1?idempotency_key=eq.'+encodeURIComponent(key),{state,provider_message_id:messageId||null,error_code:error||null,updated_at:new Date().toISOString()},'return=minimal');
}
async function persistAccepted(key,target,type,body,receipt){
  const now=new Date().toISOString();
  await sb('POST','/rest/v1/aos_wa_messages_v1?on_conflict=provider_message_id',{
    provider_message_id:receipt.message_id,idempotency_key:key,conversation_id:target.id,direction:'OUTBOUND',
    from_number:null,to_number:target.to,phone_number_id:receipt.phone_number_id||null,contact_name:null,
    message_type:type,message_body:body||null,media_id:null,status:'accepted',actor_id:ACTOR_ID,send_origin:'HUMAN',
    received_at:now,updated_at:now
  },'resolution=merge-duplicates,return=minimal');
  await sb('POST','/rest/v1/aos_wa_events_v1?on_conflict=event_key',{
    event_key:'conv-l1-test:'+key,event_type:'message.accepted',provider_message_id:receipt.message_id,status:'accepted',
    payload:{conversation_id:target.id,source:'CONV_L1_REAL_META_TEST',message_type:type,raw_content_stored:false}
...
async function sendOnce(target,suffix,payload,messageType,messageBody){
  ...
  if(!r.owner){
    const row=r.row||{};
    if(row.state==='ACCEPTED')return {kind:suffix,idempotent:true,message_id:row.provider_message_id||null,status:'ACCEPTED'};
    throw new Error('OUTBOUND_RESERVATION_'+String(row.state||'UNKNOWN'));
  }
  let out;
  try{out=await provider('POST','/api/wa/meta/dispatch-internal',{payload});}
  catch(e){throw Object.assign(new Error('PROVIDER_TRANSPORT_AMBIGUOUS'),{cause:e});}
  const d=out.data||{};
  if(out.status>=200&&out.status<300&&d.ok===true&&d.message_id){
    await markRequest(key,'ACCEPTED',String(d.message_id),null);
    await persistAccepted(key,target,messageType,messageBody,d);
    return {kind:suffix,idempotent:false,message_id:String(d.message_id),status:'ACCEPTED',provider_latency_ms:d.provider_latency_ms||0};
  }
...
function hasVariables(tpl){
  return /\{\{\s*\d+\s*\}\}/.test(JSON.stringify(tpl&&tpl.components||[]));
}
async function main(){
  assertEnv();
  ...
  const templates=await provider('GET','/api/wa/meta/templates-internal');
  const templateRows=templates.data&&Array.isArray(templates.data.templates)?templates.data.templates:[];
  ...
  const results=[];
  results.push(await sendOnce(t,'text',wa.buildOutboundPayload({to:t.to,type:'text',text:'ASCENDA L1 prueba técnica ✅. Responde “L1 OK” para validar el inbound del nuevo gateway.'}),'text','ASCENDA L1 prueba técnica'));
  results.push(await sendOnce(t,'image',wa.buildOutboundPayload({to:t.to,type:'image',link:MEDIA_URL,caption:'ASCENDA L1 media test ✅'}),'image','ASCENDA L1 media test'));
  results.push(await sendOnce(t,'interactive',interactive.buttons({to:t.to,type:'interactive_buttons',text:'ASCENDA L1 interactive test',buttons:[{id:'conv_l1_ack',title:'L1 OK'}]}),'interactive_buttons','ASCENDA L1 interactive test'));

  let chosen=null;
  if(TEMPLATE_NAME)chosen=templateRows.find(x=>x.name===TEMPLATE_NAME&&String(x.status).toUpperCase()==='APPROVED');
  if(!chosen)chosen=templateRows.find(x=>String(x.status).toUpperCase()==='APPROVED'&&!hasVariables(x));
  if(chosen){
    results.push(await sendOnce(t,'template',wa.buildOutboundPayload({to:t.to,type:'template',template_name:chosen.name,language:chosen.language||TEMPLATE_LANGUAGE}),'template',null));
  }else{
    console.log(JSON.stringify({stage:'template_send',status:'BLOCKED_NO_APPROVED_ZERO_VARIABLE_TEMPLATE'}));
  }

  console.log(JSON.stringify({stage:'send_complete',count:results.length,results:results.map(r=>({kind:r.kind,idempotent:r.idempotent,status:r.status,message_id:r.message_id,provider_latency_ms:r.provider_latency_ms||0}))}));
}
main().catch(e=>{
  console.error(JSON.stringify({stage:'fatal',error:String(e.message||e),detail:e.detail||null,safety:e.snapshot||null}));
  process.exit(2);
});
